...
- 中间提交：`feat(ffi): envelope workspace diagnostics results`
  ...
  - 验证记录：
    ...
    - `cargo fmt --check`
    - `git diff --check`
- 中间提交：`feat(ffi): envelope workspace outline snapshots`
  - 所属任务：阶段 10 的 ABI 版本、错误模型与兼容性门禁增量；把 MultiDocument workspace outline snapshot JSON result 面纳入统一结构化 envelope。
  - 提交边界：新增 `editor_core_ui_ffi_multi_document_workspace_outline_snapshot_envelope_json(MultiDocumentEditorUi*)` 和 `ECU_FEATURE_WORKSPACE_OUTLINE_SNAPSHOT_ENVELOPE` feature bit；legacy `workspace_outline_snapshot_json` 保持 raw JSON/null+last_error 语义，`apply_tab_document_symbols_json` 继续保持 status API 语义；Swift `MultiDocumentEditorUI` 新增 raw/typed envelope accessor 与 `EcuWorkspaceOutlineSnapshotEnvelope`，runtime compatibility 和 Atto optional capability report 同步新增该能力。该提交不切换 App 主路径，不替换现有 typed workspace outline snapshot wrapper，不把 document symbols apply status API 改造成 result API，也不完成完整外部 capability negotiation protocol。
  - 验证记录：
    - `cargo test -p editor-core-ui-ffi --release ffi_workspace_outline_snapshot_envelope_json_reports_success_and_errors`
    - `cargo test -p editor-core-ui-ffi --release ffi_runtime_info_json_reports_version_and_feature_descriptors`
    - `swift test --package-path swift --filter 'EditorCoreUIFFITests/testWorkspaceOutlineSnapshotEnvelope'`
    - `swift test --package-path swift --filter 'EditorCoreUIFFITests/testLoadsLibraryAndVersion'`
    - `swift test --package-path swift --filter 'EditorCoreUIFFIRuntimeCompatibilityTests/testReportsMissingRequiredFeatures'`
    - `swift test --package-path swift --filter 'AttoRuntimeCompatibilityTests/test(MissingOptionalFeaturesDoNotBlockLaunchCompatibility|ReportsMissingRequiredFeatures)'`
    - `cargo fmt --check`
...
- 中间提交：`feat(ffi): envelope multi-document snapshots`
  ...
  - 验证记录：
    ...
    - `cargo fmt --check`
    - `git diff --check`
- 中间提交：`feat(ffi): envelope multi-document search results`
  - 所属任务：阶段 10 的 ABI 版本、错误模型与兼容性门禁增量；把 MultiDocument all-tabs search JSON result 面纳入统一结构化 envelope。
  - 提交边界：新增 `editor_core_ui_ffi_multi_document_search_all_tabs_envelope_json(MultiDocumentEditorUi*, const char*, uint8_t, uint8_t, uint8_t)` 和 `ECU_FEATURE_MULTI_DOCUMENT_SEARCH_ENVELOPE` feature bit；legacy `multi_document_search_all_tabs_json` 保持 raw JSON/null+last_error 语义；Swift `MultiDocumentEditorUI` 新增 raw/typed envelope accessor 与 `EcuMultiDocumentSearchEnvelope`，runtime compatibility 和 Atto optional capability report 同步新增该能力。该提交不切换 App Find in Files 主路径，不替换现有 typed `searchAllTabs(...)` wrapper，不把其他 multi-document status/query ABI 改造成 result API，也不完成完整外部 capability negotiation protocol。
  - 验证记录：
    - `cargo test -p editor-core-ui-ffi --release ffi_multi_document_search_envelope_json_reports_success_and_errors`
    - `cargo test -p editor-core-ui-ffi --release ffi_runtime_info_json_reports_version_and_feature_descriptors`
    - `cargo test -p editor-core-ui-ffi --release ffi_feature_flags_include_semantic_tokens_requests`
    - `cargo build -p editor-core-ui-ffi --release`
    - `swift test --package-path swift --filter 'EditorCoreUIFFITests/testMultiDocumentSearchEnvelope'`
    - `swift test --package-path swift --filter 'EditorCoreUIFFITests/testLoadsLibraryAndVersion'`
    - `swift test --package-path swift --filter 'EditorCoreUIFFIRuntimeCompatibilityTests/testReportsMissingRequiredFeatures'`
    - `swift test --package-path swift --filter 'AttoRuntimeCompatibilityTests/test(MissingOptionalFeaturesDoNotBlockLaunchCompatibility|ReportsMissingRequiredFeatures)'`
    - `cargo fmt --check`
    - `git diff --check`
- 中间提交：`feat(ffi): envelope workspace roots changes`
  - 所属任务：阶段 10 的 ABI 版本、错误模型与兼容性门禁增量；把 MultiDocument workspace roots change JSON result 面纳入统一结构化 envelope。
  - 提交边界：新增 `editor_core_ui_ffi_multi_document_set_workspace_roots_with_change_envelope_json(MultiDocumentEditorUi*, const char*)` 和 `ECU_FEATURE_MULTI_DOCUMENT_WORKSPACE_ROOTS_CHANGE_ENVELOPE` feature bit；legacy `multi_document_set_workspace_roots_with_change_json` 保持 raw JSON/null+last_error 语义，status 型 `set_workspace_roots_json` 保持不变；Swift `MultiDocumentEditorUI` 新增 raw/typed envelope accessor 与 `EcuWorkspaceRootsChangeEnvelope`，runtime compatibility 和 Atto optional capability report 同步新增该能力。该提交不切换 App workspace root sync 主路径，不替换现有 typed `setWorkspaceRootsReturningChange(...)` wrapper，不改变 LSP didChange workspace folders 策略，也不完成完整外部 capability negotiation protocol。
  - 验证记录：
    - `cargo test -p editor-core-ui-ffi --release ffi_workspace_roots_change_envelope_json_reports_success_and_errors`
    - `cargo test -p editor-core-ui-ffi --release ffi_runtime_info_json_reports_version_and_feature_descriptors`
    - `cargo test -p editor-core-ui-ffi --release ffi_feature_flags_include_semantic_tokens_requests`
    - `cargo build -p editor-core-ui-ffi --release`
    - `swift test --package-path swift --filter 'EditorCoreUIFFITests/testWorkspaceRootsChangeEnvelope'`
    - `swift test --package-path swift --filter 'EditorCoreUIFFITests/testLoadsLibraryAndVersion'`
    - `swift test --package-path swift --filter 'EditorCoreUIFFIRuntimeCompatibilityTests/testReportsMissingRequiredFeatures'`
    - `swift test --package-path swift --filter 'AttoRuntimeCompatibilityTests/test(MissingOptionalFeaturesDoNotBlockLaunchCompatibility|ReportsMissingRequiredFeatures)'`
    - `cargo fmt --check`
...
- 中间提交：`feat(ffi): envelope project lsp servers`
  ...
  - 验证记录：
    ...
    - `cargo fmt --check`
    - `git diff --check`
- 中间提交：`feat(ffi): envelope editor derived snapshots`
  - 所属任务：阶段 10 的 ABI 版本、错误模型与兼容性门禁增量；把 per-`EditorUi` derived-state snapshot JSON result 面纳入统一结构化 envelope。
  - 提交边界：新增 `editor_core_ui_ffi_editor_ui_derived_snapshot_envelope_json(EditorUi*, snapshot, start, end)` 和 `ECU_FEATURE_EDITOR_UI_DERIVED_SNAPSHOT_ENVELOPE` feature bit；legacy `diagnostics_json` / `decorations_json` / `document_symbols_json` / `folding_regions_json` / `style_intervals_json` 保持 raw JSON/null+last_error 语义；Swift `EditorUI` 新增 raw/typed envelope accessor 与 `EcuDerivedSnapshotEnvelope`，runtime compatibility 和 Atto optional capability report 同步新增该能力。该提交不切换 App active diagnostics/decorations/symbols/folding/style snapshot 主路径，不替换现有 typed snapshot wrappers，不改变 derived-state 生产/缓存策略，也不完成完整外部 capability negotiation protocol。
  - 验证记录：
    - `cargo test -p editor-core-ui-ffi --release ffi_editor_ui_derived_snapshot_envelope_json_reports_success_and_errors`
    - `cargo test -p editor-core-ui-ffi --release ffi_runtime_info_json_reports_version_and_feature_descriptors`
    - `cargo test -p editor-core-ui-ffi --release ffi_feature_flags_include_semantic_tokens_requests`
    - `cargo build -p editor-core-ui-ffi --release`
    - `swift test --package-path swift --filter 'EditorCoreUIFFITests/testEditorUIDerivedSnapshotEnvelope'`
    - `swift test --package-path swift --filter 'EditorCoreUIFFITests/testLoadsLibraryAndVersion'`
    - `swift test --package-path swift --filter 'EditorCoreUIFFIRuntimeCompatibilityTests/testReportsMissingRequiredFeatures'`
    - `swift test --package-path swift --filter 'AttoRuntimeCompatibilityTests/test(MissingOptionalFeaturesDoNotBlockLaunchCompatibility|ReportsMissingRequiredFeatures)'`
    - `cargo fmt --check`
...
- 中间提交：`feat(ffi): envelope lsp workspace edit application`
  ...
  - 验证记录：
    ...
    - `cargo fmt --check`
    - `git diff --check`
- 中间提交：`feat(ffi): envelope editor minimap snapshots`
  - 所属任务：阶段 10 的 ABI 版本、错误模型与兼容性门禁增量；把 per-`EditorUi` minimap snapshot JSON result 面纳入统一结构化 envelope。
  - 提交边界：新增 `editor_core_ui_ffi_editor_ui_minimap_envelope_json(EditorUi*, start_visual_row, count)` 和 `ECU_FEATURE_EDITOR_UI_MINIMAP_ENVELOPE` feature bit；legacy `editor_core_ui_ffi_editor_ui_minimap_json(...)` 保持 raw JSON/null+last_error 语义；Swift `EditorUI` 新增 raw/typed envelope accessor 与 `EcuMinimapEnvelope`，runtime compatibility 和 Atto optional capability report 同步新增该能力。该提交不切换 App minimap 主路径，不替换 renderer RGBA/Metal/blob 查询，不改变 minimap JSON schema，也不完成完整外部 capability negotiation protocol。
  - 验证记录：
    - `cargo test -p editor-core-ui-ffi --release ffi_minimap_envelope_json_reports_success_and_errors`
    - `cargo test -p editor-core-ui-ffi --release ffi_runtime_info_json_reports_version_and_feature_descriptors`
    - `cargo test -p editor-core-ui-ffi --release ffi_feature_flags_include_semantic_tokens_requests`
    - `cargo build -p editor-core-ui-ffi --release`
    - `swift test --package-path swift --filter 'EditorCoreUIFFITests/testMinimapEnvelope'`
    - `swift test --package-path swift --filter 'EditorCoreUIFFITests/testLoadsLibraryAndVersion'`
    - `swift test --package-path swift --filter 'EditorCoreUIFFIRuntimeCompatibilityTests/testReportsMissingRequiredFeatures'`
    - `swift test --package-path swift --filter 'AttoRuntimeCompatibilityTests/test(MissingOptionalFeaturesDoNotBlockLaunchCompatibility|ReportsMissingRequiredFeatures)'`
    - `cargo fmt --check`
...
- 中间提交：`feat(ffi): envelope headless derived snapshots`
  ...
  - 验证记录：
    ...
    - `cargo fmt --check`
    - `git diff --check`
- 中间提交：`feat(ffi): envelope headless workspace results`
  - 所属任务：阶段 10 的 ABI 版本、错误模型与兼容性门禁增量；把 headless `Workspace` search-all-open-buffers 和 apply-text-edits JSON result 面纳入统一结构化 envelope。
  - 提交边界：新增 `editor_core_ffi_workspace_search_all_open_buffers_envelope_json(...)`、`editor_core_ffi_workspace_apply_text_edits_envelope_json(...)` 和 `ECF_FEATURE_WORKSPACE_RESULT_ENVELOPE` feature bit；legacy `workspace_search_all_open_buffers_json` / `workspace_apply_text_edits_json` 保持 raw JSON/null+last_error 语义；Swift `Workspace` 新增 raw/typed envelope accessor 与 `EcfWorkspaceResultEnvelope`。该提交不切换 App Find in Files 或 WorkspaceEdit 主路径，不替换 `MultiDocumentEditorUi` search/transaction envelope，不覆盖 workspace info/open/create/buffer-text/viewport-state 等查询面，也不完成完整外部 capability negotiation protocol。
  - 验证记录：
    - `cargo test -p editor-core-ffi --test abi_v1`
    - `cargo test -p editor-core-ffi --test abi_v1 workspace_result_envelope_json_reports_success_and_errors`
    - `cargo test -p editor-core-ffi --test abi_v1 feature_flags_and_alias_work`
    - `cargo test -p editor-core-ffi --test abi_v1 runtime_info_json_reports_version_and_feature_descriptors`
    - `cargo test -p editor-core-ffi --test abi_v1 public_abi_scalar_signatures_are_fixed_width`
    - `cargo build -p editor-core-ffi --release`
    - `swift test --package-path swift --filter 'WorkspaceAdditionalTests/testWorkspaceResultEnvelopesReportSuccessAndErrors'`
    - `swift test --package-path swift --filter 'WorkspaceAdditionalTests/testWorkspaceResultEnvelopeDecodesFutureFieldsAndUnknownStatus'`
    - `swift test --package-path swift --filter 'FFILibrarySmokeTests'`
    - `swift test --package-path swift --filter 'EditorCoreFFIRuntimeCompatibilityTests'`
    - `cargo fmt --check`
    - `git diff --check`
- 中间提交：`feat(ffi): envelope headless workspace queries`
  ...
  - 验证记录：
    ...
    - `cargo fmt --check`
    - `git diff --check`
- 中间提交：`feat(ffi): envelope headless workspace lifecycle`
  - 所属任务：阶段 10 的 ABI 版本、错误模型与兼容性门禁增量；把 headless `Workspace` open-buffer 和 create-view lifecycle JSON 面纳入统一结构化 envelope。
  - 提交边界：新增 `editor_core_ffi_workspace_open_buffer_envelope_json(...)`、`editor_core_ffi_workspace_create_view_envelope_json(...)` 和 `ECF_FEATURE_WORKSPACE_LIFECYCLE_ENVELOPE` feature bit；legacy `workspace_open_buffer` / `workspace_create_view` 保持 raw JSON/null+last_error 语义；Swift `Workspace` 新增 raw/typed lifecycle envelope accessor 并复用 `EcfWorkspaceResultEnvelope`。该提交不改变既有 typed `openBuffer(...)` / `createView(...)` 的返回类型或错误码，不切换 App 多文档主路径，不覆盖 processing edits bool API、LSP helper JSON 或完整外部 capability negotiation protocol。
  - 验证记录：
    - `cargo test -p editor-core-ffi --test abi_v1 workspace_lifecycle_envelope_json_reports_success_and_errors`
    - `cargo test -p editor-core-ffi --test abi_v1 feature_flags_and_alias_work`
    - `cargo test -p editor-core-ffi --test abi_v1 runtime_info_json_reports_version_and_feature_descriptors`
    - `cargo test -p editor-core-ffi --test abi_v1 public_abi_scalar_signatures_are_fixed_width`
    - `cargo test -p editor-core-ffi --test abi_v1`
    - `cargo build -p editor-core-ffi --release`
    - `swift test --package-path swift --filter 'WorkspaceTests/testWorkspaceLifecycleEnvelopes'`
    - `swift test --package-path swift --filter 'FFILibrarySmokeTests'`
    - `swift test --package-path swift --filter 'EditorCoreFFIRuntimeCompatibilityTests'`
    - `cargo fmt --check`
    - `git diff --check`
- 中间提交：`feat(ffi): envelope headless editor-state queries`
  - 所属任务：阶段 10 的 ABI 版本、错误模型与兼容性门禁增量；把 headless `EditorState` full-state、text、text-for-saving、line-ending 和 text-delta JSON query 面纳入统一结构化 envelope。
  - 提交边界：新增 `editor_core_ffi_editor_state_full_state_envelope_json(...)`、`editor_core_ffi_editor_state_text_envelope_json(...)`、`editor_core_ffi_editor_state_text_for_saving_envelope_json(...)`、`editor_core_ffi_editor_state_get_line_ending_envelope_json(...)`、`editor_core_ffi_editor_state_last_text_delta_envelope_json(...)`、`editor_core_ffi_editor_state_take_last_text_delta_envelope_json(...)` 和 `ECF_FEATURE_EDITOR_STATE_QUERY_ENVELOPE` feature bit；legacy `full_state_json` / `text` / `text_for_saving` / `get_line_ending` / `last_text_delta_json` / `take_last_text_delta_json` 保持 raw JSON/null+last_error 语义；Swift `EditorState` 新增 raw/typed query envelope accessor 并以 `EcfEditorStateQueryEnvelope` 复用 operation/value/error envelope schema。该提交不切换现有 `text()` / `fullStateJSON()` 主路径，不覆盖 derived snapshot/rendering snapshot/processing edits bool API、LSP helper JSON 或完整外部 capability negotiation protocol。
  - 验证记录：
    - `cargo test -p editor-core-ffi --test abi_v1 editor_state_query_envelope_json_reports_success_and_errors`
    - `cargo test -p editor-core-ffi --test abi_v1 feature_flags_and_alias_work`
    - `cargo test -p editor-core-ffi --test abi_v1 runtime_info_json_reports_version_and_feature_descriptors`
    - `cargo test -p editor-core-ffi --test abi_v1 public_abi_scalar_signatures_are_fixed_width`
    - `cargo test -p editor-core-ffi --test abi_v1`
    - `cargo build -p editor-core-ffi --release`
    - `swift test --package-path swift --filter 'EditorStateCoreTests/testEditorStateQueryEnvelope'`
    - `swift test --package-path swift --filter 'EditorStateCoreTests/testEditorStateQueryEnvelopeDecodesFutureFieldsAndUnknownStatus'`
    - `swift test --package-path swift --filter 'FFILibrarySmokeTests'`
    - `swift test --package-path swift --filter 'EditorCoreFFIRuntimeCompatibilityTests'`
    - `cargo fmt --check`
    - `git diff --check`
- 中间提交：`feat(ffi): envelope headless lsp helpers`
  - 所属任务：阶段 10 的 ABI 版本、错误模型与兼容性门禁增量；把 headless LSP URI/path/formatting/normalization helper JSON result 面纳入统一结构化 envelope。
  - 提交边界：新增 `editor_core_ffi_lsp_path_to_file_uri_envelope_json(...)`、`editor_core_ffi_lsp_file_uri_to_path_envelope_json(...)`、`editor_core_ffi_lsp_percent_encode_path_envelope_json(...)`、`editor_core_ffi_lsp_percent_decode_path_envelope_json(...)`、`editor_core_ffi_lsp_formatting_options_envelope_json(...)`、`editor_core_ffi_lsp_formatting_options_for_indentation_config_envelope_json(...)`、`editor_core_ffi_lsp_decode_semantic_style_id_envelope_json(...)`、`editor_core_ffi_lsp_workspace_symbols_envelope_json(...)`、`editor_core_ffi_lsp_locations_envelope_json(...)` 和 `ECF_FEATURE_LSP_HELPER_ENVELOPE` feature bit；legacy raw helper API 保持 raw JSON/null+last_error 语义；Swift `LSPBridge` 新增 raw/typed envelope accessor 并以 `EcfLSPHelperEnvelope` 复用 operation/value/error envelope schema；同步整理 `PLAN.md` 顶部 TODO 为“状态 + 阶段编号 + 一句话描述”格式。该提交不覆盖 state-mutating LSP helpers，例如 apply text edits、semantic token interval conversion、processing edit conversion、completion apply，也不切换 App 主路径或完成完整外部 capability negotiation protocol。
  - 验证记录：
    - `cargo test -p editor-core-ffi --test abi_v1 lsp_helper_envelope_json_reports_success_and_errors`
    - `cargo test -p editor-core-ffi --test abi_v1 feature_flags_and_alias_work`
    - `cargo test -p editor-core-ffi --test abi_v1 runtime_info_json_reports_version_and_feature_descriptors`
    - `cargo test -p editor-core-ffi --test abi_v1 public_abi_scalar_signatures_are_fixed_width`
    - `cargo test -p editor-core-ffi --test abi_v1`
    - `cargo build -p editor-core-ffi --release`
    - `swift test --package-path swift --filter 'LSPBridgeTests/testLSPHelperEnvelopes'`
    - `swift test --package-path swift --filter 'LSPBridgeTests/testLSPHelperEnvelopeDecodesFutureFieldsAndUnknownStatus'`
    - `swift test --package-path swift --filter 'FFILibrarySmokeTests'`
    - `swift test --package-path swift --filter 'EditorCoreFFIRuntimeCompatibilityTests'`
    - `cargo fmt --check`
    - `git diff --check`
...
- 中间提交：`test(app): cover xcui result panel navigation`
  ...
  - 验证记录：
    - `swift test --package-path swift --filter 'AttoEditorXCUIApplicationSmokeTests|AttoIPCTests|AttoCommandLineTests'`
    - `git diff --check`
- 中间提交：`test(app): cover xcui real lsp result flow`
  - 所属任务：阶段 13 的 macOS UI 自动化与视觉回归测试体系增量；把 opt-in `XCUIApplication` LSP panel flow 从 injected store seam 推进到真实 stdio LSP request/response 触发链。
  - 提交边界：`AttoEditorXCUIApplicationSmokeTests` 新增测试侧 Python stdio fixture LSP server，launch app 时只给该用例注入 `ATTO_EDITOR_LSP_CMD`；新增 real-LSP smoke flow，打开 `.rs` 文件后等待真实 `didOpen`，经 command palette 发送 `textDocument/definition` 和 `textDocument/documentSymbol` 请求，断言 Locations、Document Symbols 和 Workspace Outline panel 均由真实 server response 填充，并通过 row open/status bar position 验证目标跳转。该提交不启用默认 CI GUI 测试、不依赖外部 rust-analyzer/sourcekit-lsp、不改变产品 LSP 启动/请求语义、不提交 golden PNG。
  - 验证记录：
    - `swift test --package-path swift --filter 'AttoEditorXCUIApplicationSmokeTests|AttoIPCTests|AttoCommandLineTests'`
    - `git diff --check`
...
- 中间提交：`test(app): cover visual persistent result panels`
  ...
  - 验证记录：
    ...
    - `swift test --package-path swift --filter 'AttoEditorVisualBaselineManifestTests'`
    - `git diff --check`
- 中间提交：`test(app): cover visual hover signature popups`
  - 所属任务：阶段 13 的 macOS UI 自动化与视觉回归测试体系增量；继续扩展 popup 视觉矩阵，把 LSP hover 和 signature help popover 纳入可记录 artifact。
  - 提交边界：测试 harness 新增 `hoverPopover` / `signatureHelpPopover` manifest actions，并让 `captureTarget` 支持直接捕获 `hoverPopover` / `signatureHelpPopover` content view；manifest 新增 dark hover popover 和 dark signature help popover 两个 artifact case。该提交不提交 golden PNG、不启用 strict 默认门禁、不改变产品 UI 或运行时代码，也不覆盖 WorkspaceEdit/failure popover 或真实 LSP server 触发路径。
  - 验证记录：
    - `swift test --package-path swift --filter AttoEditorVisualBaselineManifestTests.testVisualBaselineManifestDeclaresRunnableFixtures`
    - `swift test --package-path swift --filter 'AttoEditorVisualBaselineManifestTests'`
    - `git diff --check`
- 中间提交：`test(app): cover visual problems workbench panels`
  ...
  - 验证记录：
    ...
    - `swift test --package-path swift --filter 'AttoEditorVisualBaselineManifestTests'`
    - `git diff --check`
- 中间提交：`test(app): cover visual hierarchy panel`
  - 所属任务：阶段 13 的 macOS UI 自动化与视觉回归测试体系增量；继续扩展持久在线 result panel 视觉矩阵，把 LSP Hierarchy panel 纳入可记录 artifact。
  - 提交边界：测试 harness 新增结构化 `hierarchyResults` manifest action，可生成 incoming/outgoing/type hierarchy result JSON 并复用 App 的 hierarchy snapshot + persistent panel 路径；manifest 新增 dark incoming-call Hierarchy panel artifact case。该提交不提交 golden PNG、不启用 strict 默认门禁、不改变产品 UI 或运行时代码，也不覆盖 Code Lens/Inlay/Document Links/Colors 等其它持久在线 panels。
  - 验证记录：
    - `swift test --package-path swift --filter AttoEditorVisualBaselineManifestTests.testVisualBaselineManifestDeclaresRunnableFixtures`
    - `swift test --package-path swift --filter 'AttoEditorVisualBaselineManifestTests'`
    - `git diff --check`
...
- 中间提交：`test(app): cover visual failure popovers`
  ...
  - 验证记录：
    ...
    - `swift test --package-path swift --filter 'AttoEditorVisualBaselineManifestTests'`
    - `git diff --check`
- 中间提交：`test(app): cover visual workspace edit preview`
  - 所属任务：阶段 13 的 macOS UI 自动化与视觉回归测试体系增量；继续扩展 WorkspaceEdit 视觉矩阵，把 modal WorkspaceEdit preview panel 纳入可记录 artifact。
  - 提交边界：`AttoWorkspaceEditPreviewPanelController` 新增 package-internal non-modal testing/visual entry，复用现有 panel 构建、定位、selection 和布局；visual manifest harness 新增结构化 `workspaceEditPreview` action，可从临时 workspace 文件名生成 core transaction result + preview sections；manifest 新增 dark WorkspaceEdit preview panel artifact case，覆盖 text edits、create resource operation 和 conflict detail。该提交不提交 golden PNG、不启用 strict 默认门禁、不改变 modal `runModal` 产品行为，也不覆盖真实 WorkspaceEdit failure 触发链。
  - 验证记录：
    - `swift test --package-path swift --filter AttoEditorVisualBaselineManifestTests.testVisualBaselineManifestDeclaresRunnableFixtures`
    - `swift test --package-path swift --filter 'AttoEditorVisualBaselineManifestTests'`
    - `git diff --check`
...
- 中间提交：`test(app): cover visual workspace edit failure summary`
  ...
  - 验证记录：
    ...
    - `swift test --package-path swift --filter 'AttoEditorVisualBaselineManifestTests'`
    - `git diff --check`
- 中间提交：`test(app): cover visual workspace edit undo status`
  - 所属任务：阶段 13 的 macOS UI 自动化与视觉回归测试体系增量；把 WorkspaceEdit transaction undo 的用户可见 status feedback 纳入 visual artifact matrix。
  - 提交边界：`workspaceEditJSONApplySummary` manifest action 新增可选 `undoAfterApply` / `expectedUndo` 断言，复用真实 App WorkspaceEdit JSON apply helper 和 core transaction undo hook；manifest 新增 dark editor chrome undo-status artifact case，覆盖单文档 WorkspaceEdit apply 成功后执行 transaction undo，并捕获 status bar 的 `Workspace edit undone` 反馈。该提交不提交 golden PNG、不启用 strict 默认门禁、不改变产品 WorkspaceEdit apply/undo 语义，也不覆盖多文件/global undo UI 或 rollback failure UI。
  - 验证记录：
    - `swift test --package-path swift --filter AttoEditorVisualBaselineManifestTests.testVisualBaselineManifestDeclaresRunnableFixtures`
    - `swift test --package-path swift --filter 'AttoEditorVisualBaselineManifestTests'`
    - `git diff --check`
...
- 中间提交：`test(app): cover visual workspace edit atomic rollback`
  ...
  - 验证记录：
    ...
    - `swift test --package-path swift --filter 'AttoEditorVisualBaselineManifestTests'`
    - `git diff --check`
- 中间提交：`test(app): cover visual workspace edit partial failure`
  - 所属任务：阶段 13 的 macOS UI 自动化与视觉回归测试体系增量；把 WorkspaceEdit failure visual coverage 扩展到真实 App helper + core partial runtime failure summary 触发链。
  - 提交边界：manifest 新增 dark partial runtime failure summary popover artifact case，复用二进制 invalid UTF-8 fixture 和 `workspaceEditJSONApplySummary` action，真实执行 partial apply 的跨文档 WorkspaceEdit，断言打开 tab text edit 保留已应用状态，并捕获 core transaction “partially applied” summary popover。该提交不提交 golden PNG、不启用 strict 默认门禁、不改变产品 WorkspaceEdit partial apply 语义，也不覆盖 rollback secondary failure 或专门的 global undo history UI。
  - 验证记录：
    - `swift test --package-path swift --filter AttoEditorVisualBaselineManifestTests.testVisualBaselineManifestDeclaresRunnableFixtures`
    - `swift test --package-path swift --filter 'AttoEditorVisualBaselineManifestTests'`
    - `git diff --check`
