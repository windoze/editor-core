...
public enum EcuProjectLspServersEnvelopeStatus: Hashable, Sendable {
    case success
    case error
    case unknown(String)
...
public struct EcuProjectLspServersEnvelope: Decodable, Equatable, Sendable {
    public let ok: Bool
    public let status: String
    public let value: EcuJSONValue?
    public let error: EcuProjectLspServersEnvelopeError?
    public let version: UInt32

    public var statusKind: EcuProjectLspServersEnvelopeStatus {
        EcuProjectLspServersEnvelopeStatus(rawValue: status)
    }
    ...
    public init(from decoder: Decoder) throws {
        ...
        error = try container.decodeIfPresent(EcuProjectLspServersEnvelopeError.self, forKey: .error)
        version = try container.decodeIfPresent(UInt32.self, forKey: .version) ?? 0
    }
}

public struct EcuProjectLspServersEnvelopeError: Decodable, Equatable, Sendable {
    public let code: String
    public let status: EcuStatus?
    public let message: String
...
public final class MultiDocumentEditorUI {
    ...
    public func projectLspServersEnvelopeJSON() throws -> String {
        try ffiStringResult(context: "multi_document_project_lsp_servers_envelope_json") {
            editor_core_ui_ffi_multi_document_project_lsp_servers_envelope_json(handle)
        }
    }

    public func projectLspServersEnvelope() throws -> EcuProjectLspServersEnvelope {
        try decode(
            EcuProjectLspServersEnvelope.self,
            from: projectLspServersEnvelopeJSON(),
            context: "multi_document_project_lsp_servers_envelope_decode"
        )
    }
