...
@MainActor
final class AttoCodeLensPanelController: NSObject, NSTableViewDataSource, NSTableViewDelegate, NSSearchFieldDelegate, NSWindowDelegate {
    typealias Item = AttoLspCodeLensParser.Item
    ...
    private func buildPanel() -> NSPanel {
        ...
        panel.delegate = self
        panel.identifier = NSUserInterfaceItemIdentifier(AttoAccessibilityID.codeLensPanel)

        let root = NSView(frame: .zero)
        root.identifier = NSUserInterfaceItemIdentifier(AttoAccessibilityID.codeLensPanelRoot)
        ...
        searchField.identifier = NSUserInterfaceItemIdentifier(AttoAccessibilityID.codeLensPanelSearchField)
        ...
        metadataLabel.identifier = NSUserInterfaceItemIdentifier(AttoAccessibilityID.codeLensPanelMetadataLabel)
        ...
        let column = NSTableColumn(identifier: NSUserInterfaceItemIdentifier("codeLens"))
        column.title = "Code Lens"
        column.width = 660
        ...
        tableView.identifier = NSUserInterfaceItemIdentifier(AttoAccessibilityID.codeLensPanelTable)

        scrollView.documentView = tableView
        scrollView.hasVerticalScroller = true
        scrollView.drawsBackground = false
        scrollView.identifier = NSUserInterfaceItemIdentifier(AttoAccessibilityID.codeLensPanelScrollView)
        ...
        if query.isEmpty {
            ...
            filteredRows = rows.filter { row in
                row.title.localizedCaseInsensitiveContains(query)
                    ...
                    || row.item.command?.command.localizedCaseInsensitiveContains(query) == true
            }
        }
    ...
    func tableView(_ tableView: NSTableView, viewFor tableColumn: NSTableColumn?, row: Int) -> NSView? {
        ...
        let id = NSUserInterfaceItemIdentifier(AttoAccessibilityID.codeLensPanelRow)
        ...
        label.identifier = NSUserInterfaceItemIdentifier(AttoAccessibilityID.codeLensPanelRowTitle)
    ...
    @objc private func doubleClicked(_ sender: Any?) {
        applySelectedCodeLens()
    }

    private func applySelectedCodeLens() {
        let row = tableView.selectedRow
        guard row >= 0, row < filteredRows.count else { return }
        onApply(filteredRows[row].item)
    }
