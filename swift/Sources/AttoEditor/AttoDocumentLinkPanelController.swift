...
@MainActor
final class AttoDocumentLinkPanelController: NSObject, NSTableViewDataSource, NSTableViewDelegate, NSSearchFieldDelegate, NSWindowDelegate {
    typealias Item = AttoLspDocumentLinkParser.Item
    ...
    private func buildPanel() -> NSPanel {
        ...
        panel.delegate = self
        panel.identifier = NSUserInterfaceItemIdentifier(AttoAccessibilityID.documentLinkPanel)

        let root = NSView(frame: .zero)
        root.identifier = NSUserInterfaceItemIdentifier(AttoAccessibilityID.documentLinkPanelRoot)
        root.wantsLayer = true
        root.layer?.backgroundColor = NSColor(attoHex: 0x252526, alpha: 0.98).cgColor
        root.translatesAutoresizingMaskIntoConstraints = false
        panel.contentView = root

        searchField.placeholderString = "Filter document links..."
        searchField.identifier = NSUserInterfaceItemIdentifier(AttoAccessibilityID.documentLinkPanelSearchField)
        ...
        metadataLabel.identifier = NSUserInterfaceItemIdentifier(AttoAccessibilityID.documentLinkPanelMetadataLabel)
        ...
        let column = NSTableColumn(identifier: NSUserInterfaceItemIdentifier("documentLink"))
        column.title = "Document Link"
        column.width = 720
        ...
        tableView.identifier = NSUserInterfaceItemIdentifier(AttoAccessibilityID.documentLinkPanelTable)

        scrollView.documentView = tableView
        scrollView.hasVerticalScroller = true
        scrollView.drawsBackground = false
        scrollView.identifier = NSUserInterfaceItemIdentifier(AttoAccessibilityID.documentLinkPanelScrollView)
    ...
    private func updateTitle() {
        guard let panel else { return }
        panel.title = "Document Links (\(rows.count))"
        metadataLabel.stringValue = rows.count == 1 ? "Active Tab | 1 link" : "Active Tab | \(rows.count) links"
    }
        ...
        if query.isEmpty {
            ...
            filteredRows = rows.filter { row in
                row.title.localizedCaseInsensitiveContains(query)
                    ...
                    || row.item.target?.localizedCaseInsensitiveContains(query) == true
                    || row.item.tooltip?.localizedCaseInsensitiveContains(query) == true
            }
        }
    ...
    func tableView(_ tableView: NSTableView, viewFor tableColumn: NSTableColumn?, row: Int) -> NSView? {
        ...
        let id = NSUserInterfaceItemIdentifier(AttoAccessibilityID.documentLinkPanelRow)
        ...
        label.identifier = NSUserInterfaceItemIdentifier(AttoAccessibilityID.documentLinkPanelRowTitle)
    ...
    @objc private func doubleClicked(_ sender: Any?) {
        openSelectedDocumentLink()
    }

    private func openSelectedDocumentLink() {
