...
@MainActor
final class AttoDocumentColorPanelController: NSObject, NSTableViewDataSource, NSTableViewDelegate, NSSearchFieldDelegate, NSWindowDelegate {
    typealias Item = AttoLspDocumentColorParser.Item
    ...
    private let colorForItem: (Item) -> NSColor
    ...
    init(
        ...
        colorForItem: @escaping (Item) -> NSColor,
        onOpen: @escaping (Item) -> Void
    ) {
        self.titleForItem = titleForItem
        self.colorForItem = colorForItem
        self.onOpen = onOpen
        super.init()
    }
    ...
    func update(items: [Item]) {
        rows = items.map { item in
            ...
            return Row(item: item, title: title, searchText: "\(title) \(hex)")
        }
        applyFilter()
        updateTitle()
    }
    ...
    private func buildPanel() -> NSPanel {
        ...
        panel.delegate = self
        panel.identifier = NSUserInterfaceItemIdentifier(AttoAccessibilityID.documentColorPanel)

        let root = NSView(frame: .zero)
        root.identifier = NSUserInterfaceItemIdentifier(AttoAccessibilityID.documentColorPanelRoot)
        root.wantsLayer = true
        root.layer?.backgroundColor = NSColor(attoHex: 0x252526, alpha: 0.98).cgColor
        root.translatesAutoresizingMaskIntoConstraints = false
        panel.contentView = root

        searchField.placeholderString = "Filter document colors..."
        searchField.identifier = NSUserInterfaceItemIdentifier(AttoAccessibilityID.documentColorPanelSearchField)
        ...
        metadataLabel.identifier = NSUserInterfaceItemIdentifier(AttoAccessibilityID.documentColorPanelMetadataLabel)
        ...
        let column = NSTableColumn(identifier: NSUserInterfaceItemIdentifier("documentColor"))
        column.title = "Document Color"
        column.width = 720
        ...
        tableView.identifier = NSUserInterfaceItemIdentifier(AttoAccessibilityID.documentColorPanelTable)

        scrollView.documentView = tableView
        scrollView.hasVerticalScroller = true
        scrollView.drawsBackground = false
        scrollView.identifier = NSUserInterfaceItemIdentifier(AttoAccessibilityID.documentColorPanelScrollView)
    ...
    private func position(panel: NSPanel, relativeTo window: NSWindow) {
        ...
        let height = max(panel.frame.height, 380)
    ...
    func tableView(_ tableView: NSTableView, viewFor tableColumn: NSTableColumn?, row: Int) -> NSView? {
        ...
        let id = NSUserInterfaceItemIdentifier(AttoAccessibilityID.documentColorPanelRow)
        let cell = tableView.makeView(withIdentifier: id, owner: self) as? NSTableCellView ?? NSTableCellView()
        cell.identifier = id

        let swatch = cell.subviews.compactMap { $0 as? ColorSwatchView }.first ?? ColorSwatchView(frame: .zero)
        swatch.identifier = NSUserInterfaceItemIdentifier(AttoAccessibilityID.documentColorPanelSwatch)
        swatch.color = colorForItem(rowModel.item)
        swatch.translatesAutoresizingMaskIntoConstraints = false

        let label = cell.textField ?? NSTextField(labelWithString: "")
        label.identifier = NSUserInterfaceItemIdentifier(AttoAccessibilityID.documentColorPanelRowTitle)
        label.font = NSFont.systemFont(ofSize: 13)
        label.textColor = NSColor(attoHex: 0xD4D4D4)
        label.lineBreakMode = .byTruncatingTail
        label.translatesAutoresizingMaskIntoConstraints = false

        if swatch.superview !== cell {
            cell.addSubview(swatch)
            NSLayoutConstraint.activate([
                swatch.leadingAnchor.constraint(equalTo: cell.leadingAnchor, constant: 12),
    ...
    @objc private func doubleClicked(_ sender: Any?) {
        openSelectedDocumentColor()
    }

    private func openSelectedDocumentColor() {
...
private final class ColorSwatchView: NSView {
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        NSColor(attoHex: 0x000000, alpha: 0.45).setStroke()
