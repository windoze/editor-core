...
final class AttoEditorAreaViewController: NSViewController {
    ...
    func saveActiveTab() {
        ...
        do {
            let text = try tab.editorView.editor.text()
    ...
    private func selectTab(id: UUID) {
        ...
        showTabContent(tab)
        refreshTabBar()
        attachStatusObserver(to: tab.editorView)
        updateStatusBar()
        updateWindowTitle()
        tab.editorView.window?.makeFirstResponder(tab.editorView)
    }
    ...
    func toggleMinimapForActiveTab() {
        ...
        tab.minimapContainer.showsMinimap.toggle()
        tab.minimapContainer.needsLayout = true
        tab.minimapContainer.needsDisplay = true
    }
    ...
    private func updateAlwaysPollProcessingForSelectedTab() {
        for tab in tabs {
            tab.editorView.alwaysPollProcessing = false
        }

        guard let tab = activeTab else { return }
        if (try? tab.editorView.editor.lspIsEnabled()) == true {
            tab.editorView.alwaysPollProcessing = true
        }
    }
    ...
    private func updateStatusBar() {
        ...
        let editor = tab.editorView.editor
    ...
    private func showTabContent(_ tab: AttoEditorTab) {
        contentHostView.subviews.forEach { $0.removeFromSuperview() }
        let container = tab.minimapContainer
    ...
    private func makeTab(for url: URL, isPreview: Bool) throws -> AttoEditorTab {
        ...
        try editorView.editor.setGutterWidthCells(6)
        ...
        let minimapContainer = EditorCoreSkiaMinimapContainer(
            editorView: editorView,
            showsMinimap: true,
            minimapPlacement: .rightOfScrollbar
        )
        try theme.apply(to: minimapContainer)
        ...
        editorView.onDidMutateDocumentText = { [weak self] in
...
private final class AttoEditorTab {
    ...
    var isPreview: Bool
    var isDirty: Bool
    let editorView: EditorCoreSkiaView
    let minimapContainer: EditorCoreSkiaMinimapContainer
    ...
    init(
        ...
        minimapContainer: EditorCoreSkiaMinimapContainer
    ) {
        self.id = id
        ...
        self.minimapContainer = minimapContainer
    }
}
