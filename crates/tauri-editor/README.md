...
基于 `editor-core` 的 **Tauri v2 + WebView（Vite + React）** text-grid 渲染演示。
...
该工程采用标准的 Tauri 目录结构：前端在 `src/`（Vite+React），后端在 `src-tauri/`（Rust）。
...
如果你更习惯用 `cargo run`，请先起 Vite dev server：
...
# 终端 B：Tauri app（debug 会加载 devUrl）
cargo run -p tauri-editor --features tauri-app --bin tauri-editor -- path/to/file.rs
```

或者走 release（加载 `frontendDist`）：

```bash
cd crates/tauri-editor
npm run build

cargo run -p tauri-editor --features tauri-app --bin tauri-editor --release -- path/to/file.rs
```

### 关于 `devUrl`

`crates/tauri-editor/src-tauri/tauri.conf.json` 在 debug 模式下使用 `build.devUrl`（默认 `http://localhost:1420`），
以获得 HMR 与更快的前端迭代速度；release 模式使用 `build.frontendDist`（默认 `../dist`）。

## 目录结构

- `src/`：Vite+React 前端
  - `App.tsx`：渲染编辑器 DOM 壳，并在 mount 后动态加载命令式编辑器运行时
  - `editor/app.js`：命令式编辑器运行时（text-grid 渲染、输入、IME、scrollbar/minimap/gutter 等）
  - `style.css`：编辑器样式（含折叠 gutter 的 VS Code chevron）
- `src-tauri/`：Rust/Tauri 后端（workspace member crate：`tauri-editor`）
  - `src/backend.rs`：纯 Rust 的 `EditorBackend`（Workspace + 单 view），供 Tauri 命令层调用
  - `src/render_model.rs`：`ComposedGrid` → `ViewportSnapshot`（runs 压缩 + style-set interning + 每段携带 cells 宽度）
  - `src/composed_row_index.rs`：composed rows 总数与 doc↔composed 映射
  - `src/snapshot.rs`：Rust↔JS 传输结构（serde，runs 用 tuple 形式降低 JSON 开销）
  - `src/bin/tauri-editor.rs`：Tauri v2 runnable binary（`tauri-app` feature）
  - `tauri.conf.json`：Tauri 配置（`app.withGlobalTauri=true`）
  - `capabilities/default.json`：权限（包含 `core:default`，允许打开 devtools）
...
- 如果按 `Cmd+Opt+I` 报 `webview.internal_toggle_devtools not allowed`，确认 `crates/tauri-editor/src-tauri/capabilities/default.json` 存在并包含 `core:default`（包含 `core:webview:allow-internal-toggle-devtools`）。
