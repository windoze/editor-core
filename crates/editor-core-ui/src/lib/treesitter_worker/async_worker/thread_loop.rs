...
pub(super) fn run(
    rx_worker: mpsc::Receiver<TreeSitterWorkerMsg>,
    tx_events: mpsc::Sender<TreeSitterWorkerEvent>,
) {
        ...
        match msg {
                    ...
                    Ok(mut p) => match p.sync_to(version, None, Some(&text)) {
                        Ok(_) => {
                            ...
                            debounce_deadline = Some(
                                std::time::Instant::now()
                                    + std::time::Duration::from_millis(runtime.debounce_ms as u64),
                            );
                        }
                        Err(e) => {
                            let _ = tx_events.send(TreeSitterWorkerEvent::Error(e.to_string()));
                            processor = Some(p);
                        }
                    },
                    Err(e) => {
            ...
            Err(mpsc::RecvTimeoutError::Timeout) => {
                ...
                let large_doc = runtime.prefer_visible_range_on_large_docs
                    && latest_doc_char_count >= runtime.large_doc_char_threshold as usize;
                ...
                match p.compute_processing_edits(use_range) {
                    Ok(edits) => {
                        ...
                        if dt_ms > runtime.query_budget_ms as f64 {
                            ...
                            cooldown_until = Some(
                                std::time::Instant::now()
                                    + std::time::Duration::from_millis(runtime.cooldown_ms as u64),
                            );
                        } else if degraded {
