...
/// Return the language id associated with a tab, if any.
#[unsafe(no_mangle)]
pub extern "C" fn editor_core_ui_ffi_multi_document_tab_language_id(
    multi: *mut MultiDocumentEditorUi,
    tab_id: u64,
    out_language_id_utf8: *mut *mut c_char,
) -> c_int {
    match ffi_catch(|| {
        let multi = require_mut(multi, "multi")?;
        let out_language_id = require_out_mut(out_language_id_utf8, "out_language_id_utf8")?;
        let tab_id = tab_id_from_raw(tab_id);
        if multi.is_preview_tab(tab_id).is_none() {
            return Err(format!("unknown tab id {}", tab_id.get()));
        }
        *out_language_id = multi
            .tab_language_id(tab_id)
            .map(|language_id| make_c_string_ptr(language_id.to_string()))
            .unwrap_or(ptr::null_mut());
        Ok(ECU_OK)
    }) {
        Ok(code) => {
...
/// Set or clear the language id associated with a tab.
#[unsafe(no_mangle)]
pub extern "C" fn editor_core_ui_ffi_multi_document_set_tab_language_id(
    multi: *mut MultiDocumentEditorUi,
    tab_id: u64,
    ...
    match ffi_catch(|| {
        let multi = require_mut(multi, "multi")?;
        let language_id = if language_id_utf8.is_null() {
            None
        } else {
            Some(require_str(language_id_utf8, "language_id_utf8")?.to_string())
        };
        multi
            .set_tab_language_id(tab_id_from_raw(tab_id), language_id)
            .map_err(map_ui_error)?;
        Ok(ECU_OK)
    }) {
        Ok(code) => {
