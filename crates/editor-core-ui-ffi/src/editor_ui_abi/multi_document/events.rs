...
/// Refresh and return latest multi-document unified state event sequence.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn editor_core_ui_ffi_multi_document_state_events_latest_sequence(
    multi: *mut MultiDocumentEditorUi,
    out_sequence: *mut u64,
) -> c_int {
    match ffi_catch(|| {
        ...
        unsafe {
            *out_sequence = multi.state_events_latest_sequence();
        }
        Ok(ECU_OK)
    }) {
        Ok(code) => {
...
/// Refresh and return multi-document unified state events newer than `after_sequence`.
#[unsafe(no_mangle)]
pub extern "C" fn editor_core_ui_ffi_multi_document_state_events_json(
    multi: *mut MultiDocumentEditorUi,
    after_sequence: u64,
) -> *mut c_char {
    match ffi_catch(|| {
