...
pub(crate) fn minimap_envelope_json_ptr<F>(
    surface: &'static str,
    view_id: Option<u64>,
    ...
    F: FnOnce() -> Result<Value, (EcfStatus, String)>,
{
    let envelope = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(f)) {
        Ok(Ok(value)) => {
            clear_last_error();
            minimap_envelope_success(surface, view_id, start_visual_row, count, value)
        }
        Ok(Err((status, message))) => {
            set_last_error(message.clone());
            minimap_envelope_error(surface, view_id, start_visual_row, count, status, message)
        }
        Err(_) => {
            let message = "panic across FFI boundary".to_string();
            set_last_error(message.clone());
            minimap_envelope_error(
                surface,
                view_id,
                start_visual_row,
                count,
