...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    manager.execute(Command::Cursor(CursorCommand::MoveTo { line: 1, column: 0 }))?;
    ...
    manager.execute(Command::Cursor(CursorCommand::MoveTo { line: 3, column: 0 }))?;
    manager.goto_mark("b")?;

    // Jump list: record a location before performing a jump.
    manager.push_jump_location();
    manager.execute(Command::Cursor(CursorCommand::MoveTo { line: 2, column: 0 }))?;
    manager.jump_back()?;
