...
- 2026-08-02 阶段 72 已完成：AttoEditor LSP document/workspace symbols 结果新增最近一次 `LspSymbolResultSnapshot`，结果 quick panel 会记录 typed symbols 与 placeholder；新增 `lsp.show_last_symbols` command palette / Go 菜单入口，可重新打开最近一次 symbols 结果。阶段 91 已补 bounded in-memory history；后续仍缺完整持久 Outline/Symbols panel 和 workspace symbol 增量查询。
...
- workspace symbols 已有查询输入框、quick panel 主路径、基础错误/超时/空结果反馈、最近结果 snapshot、reopen command 和 bounded in-memory history command，但还缺增量查询、持久结果面板和完整结果模型。
...
- document/workspace symbols 基础错误/超时/空结果反馈、最近结果 snapshot、reopen command 和 bounded in-memory history command 已完成；仍缺持久面板和 workspace 增量查询。
