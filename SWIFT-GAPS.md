...
- 明确 project/workspace 与 LSP server lifecycle 由 core workspace 模型协调，Swift 只负责启动参数、UI 触发和展示；当前 root diff、workspaceFolders 通知、shared root alias、project restart/shutdown/close、root 变更/session restore 后的自动启动、stderr tail 状态、项目级失败事件展示、bounded 进程健康历史、轻量 health 面板、dashboard 起点和 summary/recovery policy/inline recovery action/server grouping/trend summary、JSONL 持久化日志、按 workspace retention、大小/时间轮转、filter DSL、显式 log 查询面板、当前 workspace 清空入口/确认、当前 workspace 导出入口和 failed/exited 可配置自动重启/退避/参数调整起点已有起点，剩余重点是把更深层 ownership、恢复策略和健康视图继续下沉/产品化。
