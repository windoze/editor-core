...
- document/workspace symbols result typed payload wrapper、App 主路径 typed payload 消费、基础错误/超时/空结果反馈、最近结果 snapshot、reopen command、bounded in-memory history command、workspace symbols 增量查询面板、kind 分组/稳定排序、基础持久 Outline/Symbols panel、通用 lifecycle store 起点、typed lifecycle entry/envelope 元数据和 App 层跨 family event stream 接入已完成；持久在线 panel 已开始消费 lifecycle entry。仍缺 project/workspace 级 outline store、stale/error UI 和更完整刷新/过期策略。
