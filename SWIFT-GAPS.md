...
本阶段还把 document link resolve 的 unavailable、request failed、failed、timeout 和 resolved-without-target 接入 `AttoLspResultFeedback`，并补充 EditorCoreUI unresolved link host delegation 测试与 AttoEditor LSP-disabled resolve feedback 回归测试。剩余 auxiliary resolve 缺口主要是 inlay hint resolve 的用户可见消费，以及 document link resolve 的状态订阅驱动 stale/metadata 和 panel/history 对齐。
