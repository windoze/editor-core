...
- `EditorCoreUIFFI.MultiDocumentEditorUI` 已能访问 Rust `MultiDocumentEditorUi` 的基础 tab/preview/pin/close/split/search-all-tabs 模型，并能同步 tab 文本、保存/dirty 状态；AttoEditor session snapshot 保存路径、pane layout snapshot、opened-files/sidebar/tab-bar 投影、active-tab 读取、close-active fallback、core tab title sync、close callback URL、preview replacement close callback URL、WorkspaceEdit removed-tab callback URL、AppKit content host 投影、window title、status bar metadata、language configuration、tab group-close/close-all 命令、opened-files selection/open-existing 查找、open-with-location 导航校验、opened-scope Find in Files 结果 URL、LSP target navigation、WorkspaceEdit preview text lookup、WorkspaceEdit apply 前同步、Document Symbols / Workspace Outline URI 投影、resolved inlay hint text edit apply、Code Lens action title、active-tab reload、active-tab save、save-all dirty tabs、active-tab pin、active-pane focus/move/close、tab drag reorder、preview replacement 以及 dirty close confirmation 已开始优先消费 core tab snapshot 中的顺序、active tab、document URI、preview、dirty、view count 和 active view index。
...
- 阶段 5 后续缺口中，Swift typed model、active-tab derived-state store、status bar 消费和 Problems quick panel 已在阶段 39-41 补齐，基础持久 Outline/Symbols panel 已在阶段 115 补齐，active-tab 持久 Problems panel 已在阶段 116 补齐，workspace Problems store/panel 已在阶段 117 补齐，active-tab minimap diagnostic markers 已在阶段 118 补齐，active-tab gutter diagnostic icons 已在阶段 119 补齐，workspace diagnostics 到已打开 tab 的 marker 聚合已在阶段 120 补齐，core-owned workspace diagnostics store 起点已在阶段 123 补齐，core-backed workspace diagnostic marker snapshot 起点已在阶段 124 补齐，workspace/active diagnostics 统一 marker projection model 起点已在阶段 125 补齐，workspace/active diagnostics 状态栏摘要统一已在阶段 126 补齐，active-tab Problems quick/persistent list 的统一模型消费已在阶段 127 补齐，workspace 全局 Problems panel 已在阶段 128 补齐，diagnostics lifecycle entry 起点已在阶段 129 补齐，diagnostics events-after 查询已在阶段 130 补齐，diagnostics stale/refresh lifecycle metadata 已在阶段 131 补齐，App 层跨 result family event stream 起点已在阶段 132 补齐，code actions 已在阶段 133 接入该 event stream，completion results 已在阶段 134 接入该 event stream，rename results 已在阶段 135 接入该 event stream，document color / color presentation results 已在阶段 136 接入该 event stream，core-owned workspace diagnostics event stream 起点已在阶段 137 补齐，`EditorUi` core-owned LSP result slot event stream 起点已在阶段 138 补齐，MultiDocument/project 级 result event 聚合起点已在阶段 139 补齐，`EditorUi` core-owned LSP request lifecycle event stream 起点已在阶段 140 补齐，MultiDocument/project 级 request event 聚合起点已在阶段 141 补齐，显式 cancel/timeout lifecycle 已在阶段 142 补齐，on-type formatting request lifecycle 已在阶段 143 补齐，UI auxiliary derived-state request lifecycle 起点已在阶段 144 补齐，semantic/folding internal refresh request lifecycle 已在阶段 145 补齐，diagnostics notification/pull lifecycle 已在阶段 146 补齐，Swift LSP event metadata / workspace diagnostics 字段 typed accessor 已在阶段 147 补齐，completion result / resolve item payload typed wrapper 已在阶段 148 补齐，completion App 主路径 typed payload 消费已在阶段 149 补齐，location family result typed payload wrapper 和 App 主路径消费已在阶段 150 补齐，rename prepare/result typed payload wrapper 和 App 主路径消费已在阶段 151 补齐，code action result/resolve typed payload wrapper 和 App 主路径消费已在阶段 152 补齐，document/workspace symbols result typed payload wrapper 已在阶段 153 补齐，document color / color presentation result typed payload wrapper 已在阶段 154 补齐，call/type hierarchy result typed payload wrapper 已在阶段 155 补齐，document/workspace diagnostics pull result typed payload wrapper 和 workspace diagnostics App 主路径消费已在阶段 156 补齐，selection range result typed payload wrapper 已在阶段 157 补齐，linked editing result typed payload wrapper 已在阶段 158 补齐，code lens result/resolve typed payload wrapper 已在阶段 159 补齐，folding ranges typed payload wrapper 已在阶段 160 补齐，semantic tokens full/delta/range result typed payload wrapper、manual request/take ABI 和 App typed apply baseline 已在阶段 161 补齐；阶段 178 已补 `EditorUi` 级统一 state event drain 起点，阶段 179 已补 MultiDocument/project 级统一 state event 聚合起点，阶段 180 已补文本变更与 dirty 状态事件，阶段 181 已补 selection/caret 状态事件，阶段 182 已补 viewport 状态事件，阶段 183 已补 layout 状态事件，阶段 184 已补 derived-state changed/stale 状态事件，阶段 185 已补 AttoEditor active derived-state store 对统一 state-event cursor 的消费起点，阶段 186 已补 workspace Problems store 对 core-owned workspace diagnostics event cursor 的消费起点，阶段 187 已补 workspace diagnostic marker projection 对同一 cursor 的缓存刷新，阶段 188 已让 Locations/Symbols 持久在线 panel 消费 result lifecycle entry 而不是裸 snapshot，阶段 189 已把这些 lifecycle metadata 显示到持久在线 panel 的稳定 UI 文本，阶段 190 已给 Locations/Symbols lifecycle entry 增加 Fresh/Stale/Error 状态模型，持久在线 panel metadata 可显示状态，并在 active 文档编辑后把当前 Locations/Symbols 结果标成 `Stale: document edited`，新结果记录默认恢复 Fresh；阶段 191 已把 Locations/Symbols 显式 request feedback error 接到 panel current entry，阶段 192 已补 project-level request/result event cursor 的 panel error 聚合起点，阶段 193 已补 opened-document workspace outline store 与 panel command 起点，阶段 194 已补 core-owned workspace outline snapshot ABI/Swift wrapper 与 App 消费，阶段 195 已补打开 tab 的 core document URI metadata，阶段 196 已补 `MultiDocumentEditorUi` open-tab WorkspaceEdit transaction preview/apply 起点，阶段 197 已补 WorkspaceEdit transaction event cursor 起点，阶段 198 已补 open-tab resource operations transaction 起点，阶段 199 已补 skipped/conflict detail 起点，阶段 200 已补 open-tab TextDocumentEdit version check 起点，阶段 203 已补 root-gated 未打开本地文件 text edits，阶段 204 已补 root-gated 未打开本地文件 resource operations，阶段 205 已补 documentChanges 顺序保持 apply，阶段 206 已补未打开本地 resource operation 运行时 filesystem error rollback 起点，阶段 207 已补打开 tab 本地 resource operation 的 root-gated 文件系统副作用，阶段 208 已把 AttoEditor WorkspaceEdit App apply helper 主路径切到 core transaction，阶段 209 已补基础 preview/confirmation 起点，阶段 210 已补专用 diff preview panel 起点，阶段 211 已补未打开文件 text edit filesystem rollback，阶段 212 已补打开 tab transaction failure rollback，阶段 213 已补显式 atomic apply mode，阶段 214 已补打开 tab App 投影 undo grouping 起点，阶段 215 已补 atomic apply 运行时 text edit failure rollback，阶段 216 已补 WorkspaceEdit resource-order dependency preflight，阶段 217 已补 unsupported resource operation 的 ordered dependency preflight，阶段 218 已补 WorkspaceEdit resource operation typed summary，阶段 219 已补 WorkspaceEdit dirty/conflict typed summary，阶段 220 已补最近一次 WorkspaceEdit transaction undo 起点，阶段 221 已补用户级 command/menu/keymap 起点，阶段 222 已补 session snapshot 的 core tab projection 起点，阶段 223 已补 opened-files/sidebar/tab-bar 的 core tab projection 起点，阶段 224 已补 active-tab 查询的 core active tab projection 起点，阶段 225 已补 AppKit content host 的 core active tab projection 起点，阶段 226 已补 close-other/close-right tab group 命令的 core tab projection 起点，阶段 227 已补 close-all tab 命令的 core tab projection 起点，阶段 228 已补 opened-files selection/open-existing 的 core document URI projection 起点，阶段 229 已补 open-with-location 导航校验的 core document URI projection 起点。阶段 345 已补 bounded 多级 WorkspaceEdit transaction-wide undo stack，阶段 346 已补 AppKit `UndoManager` undo action 起点，阶段 347 已补 WorkspaceEdit transaction redo，阶段 348 已补 WorkspaceEdit conflict preview 分组和 partial apply 按钮提示，阶段 349 已补 core conflict impact/resolution typed 语义和 atomic conflict 禁用应用按钮，阶段 350 已补 App apply 路径的 blocking conflict 硬 gate 与 transaction envelope parser 兼容。剩余缺口收窄为更完整跨 tab/project panel UI、更深层 conflict 检测/解决 UI。
- 阶段 351 继续补齐 WorkspaceEdit conflict resolution UI 起点：preview panel 现在有 `Open Conflict` 操作，可打开/选中冲突目标 tab；该阶段未包含保存/丢弃、重跑请求、重新 apply 和跨 transaction/project-level conflict owner。
- 阶段 352 继续补齐 WorkspaceEdit conflict resolution UI 起点：preview panel 现在对 `dirty_document` / `save_or_discard` conflict 有 `Save Conflict` 操作，可复用既有 save-tab 路径保存冲突目标 tab，并让用户保存后重试同一 WorkspaceEdit transaction；该阶段未包含 discard、自动重跑请求、自动重新 apply 和跨 transaction/project-level conflict owner。
- 阶段 353 继续补齐 WorkspaceEdit conflict resolution UI 起点：preview panel 现在对 `dirty_document` / `save_or_discard` conflict 有 `Discard Conflict` 操作，可从磁盘重载冲突目标 tab、丢弃未保存变更并同步 core dirty 状态，再让用户重试同一 WorkspaceEdit transaction；该阶段未包含自动重跑请求、自动重新 apply 和跨 transaction/project-level conflict owner。
- 阶段 354 继续补齐 WorkspaceEdit conflict resolution UI：preview panel 现在对 `dirty_document` / `save_or_discard` conflict 有 `Save & Retry` / `Discard & Retry` 操作，可在保存/丢弃冲突目标后自动重新同步、重新 preview 同一 WorkspaceEdit JSON，并在用户确认后继续 core transaction apply；仍缺真正重跑 LSP request 以生成新 WorkspaceEdit payload，以及跨 transaction/project-level conflict owner。
- 阶段 369 继续补齐 WorkspaceEdit request retry ownership 起点：rename result apply 路径现在会为 WorkspaceEdit preview 提供 request retry owner，`Save & Retry` / `Discard & Retry` 在解决 dirty conflict 后会重新发起原始 `textDocument/rename`，由 server 生成新的 WorkspaceEdit payload；无 request owner 的手工 WorkspaceEdit JSON 路径继续复用旧 payload。该阶段未覆盖 code action、completion additional edits、code lens command 等其他 WorkspaceEdit source 的 request owner，也未覆盖跨 transaction/project-level conflict owner。
- 阶段 370 继续补齐 WorkspaceEdit request retry ownership 起点：code action result/resolve 的 WorkspaceEdit apply 路径现在会传递原始 code action request context，解决 dirty conflict 后可重新发起 `textDocument/codeAction` 请求；无 request context 的直接 apply 路径继续复用旧 payload。仍缺 completion additional edits、code lens command 等其他 WorkspaceEdit source 的 request owner，以及跨 transaction/project-level conflict owner。
- 阶段 371 继续补齐 WorkspaceEdit request retry ownership 与 completion additional edits transaction 起点：普通（非 snippet）completion 的 main text edit + `additionalTextEdits` 现在会转换为当前文档 WorkspaceEdit JSON 并走 AttoEditor 既有 core transaction apply/preview/retry 入口；completion request context 记录原始 logical line/column，解决 dirty conflict 后可重新发起 `textDocument/completion`。snippet completion 仍保留 `applySnippet(...)` 以维护 placeholder session。仍缺 code lens command 等其他 WorkspaceEdit source 的 request owner、跨 transaction/project-level conflict owner，以及 snippet placeholder 与 transaction 的更深合并语义。
- 阶段 372 继续补齐 WorkspaceEdit request retry ownership 与 Code Lens command source：`workspace/executeCommand` result 中的 WorkspaceEdit 现在会进入 AttoEditor 既有 core transaction apply/preview/retry 入口；Code Lens command 因此可在 dirty conflict 解决后重发原始 `workspace/executeCommand`，普通非 WorkspaceEdit command result 仍显示 HUD。仍缺跨 transaction/project-level conflict owner、snippet placeholder 与 transaction 的更深合并语义和更深层 conflict 检测/解决 UI。
- 阶段 373 继续补齐 WorkspaceEdit project-level conflict owner 起点：WorkspaceEdit History panel 现在会从 core transaction event 的 typed `conflicts` 字段显示 conflict 数量和首个目标 URI，并提供 `Open Conflict` 按钮复用既有目标打开逻辑；这让 partial/rejected transaction 的冲突不再只停留在一次性 preview 中。仍缺更完整跨 transaction/project-level conflict owner、snippet placeholder 与 transaction 的更深合并语义和更深层 conflict 检测/解决 UI。
- 阶段 374 继续补齐 WorkspaceEdit project-level conflict owner 起点：WorkspaceEdit History panel 现在会对 core event conflict 中 `resolution == "save_or_discard"` 或 `kind == "dirty_document"` 的目标启用 `Save Conflict` / `Discard Conflict`，复用 preview panel 已有保存/丢弃目标逻辑；历史记录中的 partial/rejected transaction 已可触达手动解决动作。仍缺更完整跨 transaction/project-level conflict owner、自动 retry/reapply 编排、snippet placeholder 与 transaction 的更深合并语义和更深层 conflict 检测/解决 UI。
- 阶段 375 继续补齐 WorkspaceEdit request retry ownership：resolved inlay hint 的 `textEdits` WorkspaceEdit 现在会保留原始 hint JSON 并传入 `inlayHint/resolve` request retry owner；`Save & Retry` / `Discard & Retry` 在解决 dirty conflict 后可重新发起原始 resolve 请求，由 server 重新生成 resolved hint payload。仍缺更完整跨 transaction/project-level conflict owner、history transaction 自动 retry/reapply 编排、snippet placeholder 与 transaction 的更深合并语义和更深层 conflict 检测/解决 UI。
- 阶段 376 继续补齐 WorkspaceEdit request retry ownership 与 color presentation edits transaction 起点：color presentation 的 `textEdit` / `additionalTextEdits` 现在会转换为当前文档 WorkspaceEdit JSON 并走 AttoEditor 既有 core transaction apply/preview/retry 入口；原始 color item 作为 request context 传给自动应用和 palette action，解决 dirty conflict 后可重新发起 `textDocument/colorPresentation`。仍缺更完整跨 transaction/project-level conflict owner、history transaction 自动 retry/reapply 编排、snippet placeholder 与 transaction 的更深合并语义和更深层 conflict 检测/解决 UI。
- 阶段 377 继续补齐 WorkspaceEdit request retry ownership 与 formatting transaction 起点：显式 Format Document / Format Selection 现在通过 raw `textDocument/formatting` / `textDocument/rangeFormatting` request/take JSON 获取 `TextEdit[]`，转换为当前文档 WorkspaceEdit JSON 后走 AttoEditor 既有 core transaction apply/preview/retry 入口；解决 dirty conflict 后可重新发起原始 formatting request。旧 blocking formatting helper 仍保留给 format-on-save、on-type 和兼容调用。仍缺更完整跨 transaction/project-level conflict owner、history transaction 自动 retry/reapply 编排、snippet placeholder 与 transaction 的更深合并语义和更深层 conflict 检测/解决 UI。
- 阶段 378 继续补齐 WorkspaceEdit history transaction reapply 编排：core WorkspaceEdit transaction event 现在会兼容携带原始 `workspace_edit_json`，Swift typed event optional decode 后供 WorkspaceEdit History panel 显式 `Reapply`；History panel 也为 `Rejected` 的 dirty/save-or-discard conflict 事务提供 `Save & Reapply` / `Discard & Reapply`，先复用既有保存/丢弃目标动作再重新应用同一事务 payload。partial transaction 的普通 save/discard 行为保持手动解决，避免自动重放时重复已应用 text edits。仍缺更完整跨 transaction/project-level request owner、snippet placeholder 与 transaction 的更深合并语义和更深层 conflict 检测/解决 UI。
- 阶段 379 继续补齐 WorkspaceEdit history request owner 编排：AttoEditor 现在会把成功记录的 core WorkspaceEdit transaction sequence 映射到 session-local request retry owner，WorkspaceEdit History panel 可显示 retry label 并提供 `Rerun Request`；对带 owner 的 rejected dirty/save-or-discard conflict，历史面板的 `Save & Reapply` / `Discard & Reapply` 会切换为 `Save & Rerun` / `Discard & Rerun`，先解决目标再重新发起原请求。owner 会随当前 core history sequence 修剪，不跨 session/project 持久化；无 owner 记录继续使用原始 payload reapply。仍缺真正跨 session/project-level request owner、snippet placeholder 与 transaction 的更深合并语义和更深层 conflict 检测/解决 UI。
- 阶段 380 继续补齐 snippet completion 与 WorkspaceEdit transaction 的合并起点：snippet completion 的 `additionalTextEdits` 现在会先转换为当前文档 WorkspaceEdit JSON 并走 core transaction apply/preview/retry 入口，再按前置 additional edits 的 char delta 转换 snippet 主编辑范围并调用 core snippet engine，以保留 placeholder session。该阶段仅覆盖当前文档、非交叠 additional edits；无法安全转换时继续回退到旧 `applySnippet(..., additionalEdits:)` 路径。仍缺真正跨 session/project-level request owner、snippet 主体与 transaction 的统一 undo/跨文件合并语义和更深层 conflict 检测/解决 UI。
- 阶段 381 进一步补齐阶段 5 的 project/root 投影：AttoEditor 同步 project LSP launch metadata 到 `MultiDocumentEditorUI.projectLspServers` 时，`workspaceRoots` 现在优先来自 core `MultiDocumentEditorUI.snapshot().workspaceRoots`，只在 core snapshot 失败或为空时回退 Swift 窗口当前 root；该完成项属于 `PLAN.md` 阶段 5 的 `feat(app): project lsp launch roots from core workspace` 增量，仍不改变实际 LSP server 启停路径或 typed lifecycle ownership。
- 阶段 382 继续补齐阶段 6 的 core-owned project LSP typed lifecycle plan 起点：`MultiDocumentEditorUi` 现在可以基于打开 tab 的 `document_uri` / `language_id`、core workspace roots 和 project LSP server configs 生成 project LSP auto-start plan；UI FFI/Swift wrapper 可 typed 查询该计划，runtime feature flags 也暴露了该能力。当前仍只是 plan/query 面，不实际启动、停止或重启 LSP server，不完成 shared-session ownership、跨独立 project session 合并或 dashboard 产品化。
- 阶段 383 继续补齐阶段 6 的 App 消费 core start-plan 起点：AttoEditor project/root 自动启动 LSP 时，会先把 registry/env/保存 launch config 解析出的候选投影到 `MultiDocumentEditorUI.projectLspServers`，再用 `projectLspStartPlan()` 决定可启动 tab；document URI、tab id 和 server key 不匹配的候选不会启动，root URI 优先来自 plan entry。当前仍不改变手动 restart/auto-restart 路径，不把 LSP server 进程 ownership 下沉到 core。
- 阶段 384 继续补齐阶段 6 的 project LSP lifecycle outcome 记录起点：`MultiDocumentEditorUi` 新增 bounded typed lifecycle event stream，可记录 project LSP start 的 started/failed outcome；UI FFI/Swift wrapper 可 record/query/latest sequence，AttoEditor project/root 自动启动路径会在既有 `EditorUI.lspEnable(...)` 成功或失败后把 outcome 写回 core。当前仍不改变手动 restart/auto-restart/stop 路径，不把 LSP server 进程 ownership 或恢复策略执行下沉到 core。
- 阶段 385 继续补齐阶段 6 的 project LSP lifecycle outcome App 消费：AttoEditor project lifecycle drain 现在会读取 core `projectLspLifecycleEvents(after:)`，把 start outcome 缓存到 bounded App store，并在 Project Status Events panel 与 Project Dashboard 中展示 lifecycle 计数和明细。当前仍不改变 LSP 启动/停止/restart 执行路径，不把进程 ownership、恢复策略执行或跨 project session 策略下沉到 core。
- 阶段 386 继续补齐阶段 6 的 project LSP restart outcome 记录：core lifecycle outcome payload 现在支持 `operation = "restart"`；AttoEditor active-tab restart、project-level restart 和 auto-restart 成功/失败后会把 restart outcome 写回 core stream，Project Status Events panel / Dashboard 可通过既有 drain 展示。当前仍不改变 restart 执行路径，不记录 stop/shutdown outcome，不把进程 ownership 或恢复策略执行下沉到 core。
- 阶段 387 继续补齐阶段 6 的 project LSP stop outcome 记录：core lifecycle outcome payload 现在支持 `operation = "stop"` / `status = "stopped"`；AttoEditor 关闭拥有 LSP session 的 tab 和显式切换到 Plain Text/Tree-sitter 语言前，会把用户级 stop outcome 写回 core stream，Project Status Events panel / Dashboard 可通过既有 drain 展示。当前仍不改变 stop 执行路径，不记录内部 restart 前置 disable 或显式 `lspShutdown()` UI 路径，不把进程 ownership 或恢复策略执行下沉到 core。
- 阶段 388 继续补齐阶段 6 的 host-visible LSP shutdown App 入口：AttoEditor 现在提供 `lsp.shutdown_server` command/Go 菜单，调用 `EditorUI.lspShutdown()` graceful shutdown 当前 tab LSP session；成功后保留 launch config、抑制自动重启并记录 `operation = "stop"` / `trigger = "manual_shutdown"` / `status = "stopped"` 到 core lifecycle stream。当前仍不实现 project-wide shutdown、进程 ownership 下沉或 shared-session 完整策略。
- 阶段 389 继续补齐阶段 6 的 project-level LSP shutdown App 入口：AttoEditor 现在提供 `lsp.shutdown_project_servers` command/Go 菜单，按 core-projected open tabs graceful shutdown 当前 workspace 中运行且已配置的 LSP sessions；成功后保留 launch config、抑制自动重启并记录 `operation = "stop"` / `trigger = "project_shutdown"` / `status = "stopped"` 到 core lifecycle stream。当前仍不把实际 shutdown 执行、shared-session 去重/合并或跨 project ownership 下沉到 core。
- 阶段 390 继续补齐阶段 6 的 core-owned project LSP stop/shutdown plan 查询面：`MultiDocumentEditorUi` 现在可以基于打开 tab 的 `document_uri` / `language_id` 和 project LSP server configs 生成 stop plan，UI FFI/Swift wrapper 可 typed 查询，AttoEditor project-level shutdown 路径会先消费该 plan 再调用 `EditorUI.lspShutdown()`。当前仍不把实际 shutdown 执行、shared-session 去重/合并或跨 project ownership 下沉到 core。
- 阶段 391 继续补齐阶段 6 的 core-owned project LSP restart plan 查询面：`MultiDocumentEditorUi` 现在可以基于打开 tab 的 `document_uri` / `language_id` 和 project LSP server configs 生成 restart plan，UI FFI/Swift wrapper 可 typed 查询，AttoEditor project-level restart 路径会先消费该 plan 再调用既有 `EditorUI.lspDisable()` / `EditorUI.lspEnable(...)` restart pipeline。当前仍不把实际 restart 执行、shared-session 去重/合并或跨 project ownership 下沉到 core。
- 阶段 392 继续补齐阶段 6 的 App 消费 core restart-plan 覆盖面：AttoEditor failed/exited 自动恢复路径现在也会先消费 `MultiDocumentEditorUI.projectLspRestartPlan()`，core plan 命中时 root URI 来自 plan entry，再调用既有 restart pipeline。当前仍不把实际 auto-restart 执行、退避策略、shared-session 去重/合并或跨 project ownership 下沉到 core。
- 阶段 393 继续补齐阶段 6 的 App 消费 core restart-plan 覆盖面：AttoEditor active-tab 手动 restart 现在也会先消费 `MultiDocumentEditorUI.projectLspRestartPlan()`，core plan 命中时 root URI 来自 plan entry，再调用既有 restart pipeline。当前仍不把实际 restart 执行、shared-session 去重/合并或跨 project ownership 下沉到 core。
- 阶段 394 继续补齐阶段 6 的 App 消费 core stop-plan 覆盖面：AttoEditor active-tab 手动 shutdown 现在也会先消费 `MultiDocumentEditorUI.projectLspStopPlan()`，core plan 命中后再调用既有 `EditorUI.lspShutdown()` graceful shutdown。当前仍不把实际 shutdown 执行、shared-session 去重/合并或跨 project ownership 下沉到 core。
- 阶段 395 继续补齐阶段 6 的 core plan gate 可观测性：core project LSP lifecycle stream 现在支持 `status = "skipped"`，AttoEditor active-tab manual restart/shutdown 被 restart/stop plan gate 拒绝时会写入 skipped lifecycle event，再展示 unavailable UI。当前仍不把实际 restart/shutdown 执行、shared-session 去重/合并或跨 project ownership 下沉到 core。
- 阶段 396 继续补齐阶段 6 的 core lifecycle 执行状态节点：core project LSP lifecycle stream 现在支持 `status = "requested"`，AttoEditor active-tab manual restart/shutdown 通过 restart/stop plan gate 后、调用 Swift wrapper 执行前会写入 requested lifecycle event，执行后继续写 started/stopped/failed 终态。当前仍不把实际 restart/shutdown 执行、shared-session 去重/合并或跨 project ownership 下沉到 core。
- 阶段 397 继续补齐阶段 6 的 core lifecycle attempt correlation：core project LSP lifecycle event 现在有兼容 optional `attempt_id`，requested event 会默认用自己的 sequence 作为 attempt id，AttoEditor active-tab manual restart/shutdown 的终态 outcome 会带回同一 attempt id，plan gate skipped 仍不生成 attempt id。当前仍不把实际 restart/shutdown 执行、shared-session 去重/合并或跨 project ownership 下沉到 core。
- 阶段 398 继续补齐阶段 6 的 project-level lifecycle attempt correlation：AttoEditor project-level restart/shutdown 批量入口现在会对每个 core plan 命中的目标先写 requested lifecycle event，再让 started/stopped/failed 终态 outcome 带回同一 `attempt_id`。当前仍不把实际 restart/shutdown 执行、shared-session 去重/合并或跨 project ownership 下沉到 core。
- 阶段 399 继续补齐阶段 6 的 auto-restart lifecycle attempt correlation：AttoEditor failed/exited 自动恢复路径现在会在 restart plan gate 允许后先写 auto_restart requested lifecycle event，再让 started/failed 终态 outcome 带回同一 `attempt_id`。当前仍不把实际 restart 执行、退避策略、shared-session 去重/合并或跨 project ownership 下沉到 core。
- 阶段 400 继续补齐阶段 6 的 auto-start lifecycle attempt correlation：AttoEditor project/root 自动启动路径现在会在 start plan 命中后先写 auto_start requested lifecycle event，再让 started/failed 终态 outcome 带回同一 `attempt_id`。当前仍不把实际 start 执行、shared-session 去重/合并或跨 project ownership 下沉到 core。
- 阶段 401 继续补齐阶段 6 的用户级 stop lifecycle attempt correlation：AttoEditor tab close 和 language change stop 路径现在会在 `EditorUI.lspDisable()` 前写 requested lifecycle event，再让 stopped 终态 outcome 带回同一 `attempt_id`。当前仍不把实际 stop 执行、shared-session 去重/合并或跨 project ownership 下沉到 core。
- 阶段 402 继续补齐阶段 6 的 lifecycle attempt 可见性：Project Status Events panel 和 Project Dashboard 的 lifecycle 行现在会在事件包含 `attempt_id` 时显示 `attempt #...`，方便关联 requested 与终态 outcome。当前仍不把实际启停执行、shared-session 去重/合并或跨 project ownership 下沉到 core。
- 阶段 403 继续补齐阶段 6 的 lifecycle attempt dashboard 汇总：Project Dashboard summary 现在同时显示 raw lifecycle event 数和按 `attempt_id` 去重后的 lifecycle attempt 数，减少 requested/终态双事件后的计数歧义。当前仍不把实际启停执行、shared-session 去重/合并或跨 project ownership 下沉到 core。
- 阶段 404 继续补齐阶段 7 的 LSP Workbench lifecycle summary：Workbench item 现在有结构化 lifecycle state，metadata 会汇总当前 result family 的 stale/error 数，减少目录层排查时逐行扫描状态文本。当前仍不新增 clear/pin/history 操作，也不迁移到真正内嵌 dock。
- 阶段 405 继续补齐阶段 7 的 LSP Workbench clear stale 起点：AttoEditor 现在提供 `lsp.clear_workbench_stale_results` command/菜单，能把 Workbench 中 stale 的 Locations/Symbols、diagnostics 和 event-backed auxiliary family 状态恢复为 Fresh，同时保留当前结果内容。当前仍不实现 pin/history 模型，也不迁移到真正内嵌 dock。
- 阶段 406 继续补齐阶段 7 的 LSP Workbench pin result 起点：AttoEditor 现在提供 `lsp.pin_workbench_current_results` command/菜单，能把当前 Workbench 中 Locations、Symbols、Workspace Outline、active/workspace diagnostics 和 event-backed auxiliary family 的最近结果标记为 session-local pinned，并在 Workbench metadata 中汇总 pinned 数。当前仍不持久化 pinned 状态，不新增 unpin/history UI，也不迁移到真正内嵌 dock。
- 阶段 407 继续补齐阶段 7 的 LSP Workbench history entries 可见性：Workbench item 现在携带 `historyCount`，metadata 会汇总 Locations、Symbols、Workspace Outline、diagnostics 和 event-backed auxiliary family 的 bounded result history 条目数。当前仍不新增统一 history panel，不持久化 history，也不迁移到真正内嵌 dock。
- 阶段 408 继续补齐阶段 7 的 LSP Workbench jump navigation 起点：Workbench item 现在携带 `jumpTargetCount`，metadata 会汇总可跳转目标数；AttoEditor 提供 `lsp.jump_workbench_first_result` command/菜单，按 Locations、Symbols、Workspace Outline、Hierarchy 的当前结果优先级跳转首个 target。当前仍不新增 row-level secondary action，不覆盖 diagnostics/document-links/code-lens 的专用打开/执行语义，也不迁移到真正内嵌 dock。
- 阶段 409 继续补齐阶段 7 的 Document Colors refresh 操作：AttoEditor 现在提供 `lsp.refresh_document_colors` command/菜单和 refresh-only result mode，成功后只更新颜色缓存、可见颜色面板、Workbench 和 `document_colors` lifecycle event，不主动打开 quick panel、color picker 或 panel。当前仍不实现跨 tab/project Colors history，也不迁移到真正内嵌 dock。
- 阶段 410 继续补齐阶段 7 的 Hierarchy panel refresh 操作：AttoEditor 现在会在成功的 call/type hierarchy children 请求后记录 session-local 可刷新 root item，并提供 `lsp.refresh_hierarchy_panel` command/菜单；刷新成功只更新 hierarchy snapshot、可见 Hierarchy panel、Workbench 和 `hierarchy` lifecycle event，不主动打开 quick result panel。当前仍不实现树状展开、row-level children refresh、跨 tab/project Hierarchy history，也不迁移到真正内嵌 dock。
- 阶段 411 继续补齐阶段 7 的 LSP Workbench pin result 反向操作：AttoEditor 现在提供 `lsp.unpin_workbench_current_results` command/菜单，能清除 Locations、Symbols、Workspace Outline、active/workspace diagnostics 和 event-backed auxiliary family 的 session-local Workbench pin，并刷新已打开 Workbench。当前仍不新增 pinned history panel，不持久化 pinned 状态，也不迁移到真正内嵌 dock。
- 阶段 412 继续补齐阶段 7 的 Hierarchy panel 可展开起点：Hierarchy result entry 现在保留源 item `requestJSON`，AttoEditor 提供 `lsp.expand_hierarchy_selection` command/菜单，能沿用最近 hierarchy kind 对当前 panel 选中行继续请求 children；panel 右箭头也会触发展开回调。当前仍不实现真正树状嵌套 UI、不持久化 expanded state、不新增跨 tab/project Hierarchy history，也不迁移到真正内嵌 dock。
- 阶段 413 继续补齐阶段 7 的 LSP Workbench history panel 起点：AttoEditor 现在提供 `lsp.show_workbench_history` command/菜单和 `AttoLspWorkbenchHistoryPanelController`，可过滤查看 Locations、Symbols、Workspace Outline、diagnostics 与 event-backed auxiliary family 的 bounded session history，并显示状态、sequence、数量和 pinned 标记。当前仍不持久化 result history，不实现跨 tab/project history store，也不迁移到真正内嵌 dock。
- 阶段 414 继续补齐阶段 7 的 LSP Workbench history row restore 起点：history item 现在携带 `resultSequence`，AttoEditor 打开 Locations、Symbols、Workspace Outline 与 active/workspace diagnostics 历史行时会按 sequence 恢复对应 session-local lifecycle entry，再打开对应持久在线 panel。当前 event-backed auxiliary family 仍不保存完整可恢复 snapshot，不重放 LSP request，不持久化 result history，也不迁移到真正内嵌 dock。
- 阶段 415 继续补齐阶段 7 的 LSP Workbench auxiliary history restore 起点：AttoEditor 现在按 result event sequence bounded 保存 Code Lens、Inlay Hints、Document Links、Document Colors 和 Hierarchy 的 session-local panel item/snapshot，history row restore 可恢复这些 event-backed auxiliary family 的旧 snapshot 并展示对应 panel。当前仍不把 result history 下沉到 core-owned project store，不持久化或跨 tab/project 共享，不重放 LSP request，也不迁移到真正内嵌 dock。
- 阶段 416 继续补齐阶段 7 的 LSP Workbench selected history 入口：Workbench panel 现在保留稳定选中项，右箭头二级操作和 `lsp.show_workbench_selected_history` command/菜单可按当前选中 result family 打开过滤后的 history panel。当前仍不把 result history 下沉到 core-owned project store，不持久化或跨 tab/project 共享，不重放 LSP request，也不迁移到真正内嵌 dock。
- 阶段 417 继续补齐阶段 7 的 LSP Workbench selected jump 入口：AttoEditor 现在提供 `lsp.jump_workbench_selected_result` command/菜单，按 Workbench 当前选中 result family 跳转 Locations、Symbols、Workspace Outline 或 Hierarchy 的首个 target，并让全局首个结果跳转复用同一 family target helper。当前仍不覆盖 diagnostics/document-links/code-lens 的专用打开/执行语义，不把 result history 下沉到 core-owned project store，不持久化或跨 tab/project 共享，也不迁移到真正内嵌 dock。
- 阶段 418 继续补齐阶段 7 的 LSP Workbench selected refresh 入口：AttoEditor 现在提供 `lsp.refresh_workbench_selected_result` command/菜单，按 Workbench 当前选中 result family 分发到 Workspace Problems、Code Lens、Inlay Hints、Document Links、Document Colors 或 Hierarchy 的既有刷新入口；Locations/Symbols/Workspace Outline 等没有 request context 的 family 保持不可刷新并显示状态。当前仍不伪造 Locations/Symbols refresh context，不把 result history 下沉到 core-owned project store，不持久化或跨 tab/project 共享，也不迁移到真正内嵌 dock。
- 阶段 419 继续补齐阶段 7 的 LSP Workbench selected pin/unpin 入口：AttoEditor 现在提供 `lsp.pin_workbench_selected_result` / `lsp.unpin_workbench_selected_result` command/菜单，按 Workbench 当前选中 result family 操作现有 session-local pinned 状态；全量 pin/unpin 也复用同一 family helper。当前仍不持久化 pinned 状态，不跨 tab/project 共享，不删除 history，不重新请求 LSP，也不迁移到真正内嵌 dock。
- 阶段 420 继续补齐阶段 7 的 LSP Workbench selected clear stale 入口：AttoEditor 现在提供 `lsp.clear_workbench_selected_stale_result` command/菜单，按 Workbench 当前选中 result family 清除现有 session-local stale 状态；全量 clear stale 也复用同一 family helper，并且 lifecycle store 能按指定历史 entry 更新 stale/fresh 状态。当前仍不删除 result 内容，不清理 history，不持久化 stale 状态，不重新请求 LSP，也不迁移到真正内嵌 dock。
- 阶段 421 继续补齐阶段 7 的 LSP Workbench pinned results 可见入口：AttoEditor 现在提供 `lsp.show_workbench_pinned_results` command/菜单，复用 Workbench History panel 展示当前 session-local pinned result entries，并保留 history row restore 能力；可见 history panel 刷新时会保留 pinned-only 模式。当前仍不持久化 pinned 状态，不跨 tab/project 共享，不新增独立 pinned store，不重新请求 LSP，也不迁移到真正内嵌 dock。
- 阶段 422 修正阶段 7 的 LSP Workbench pinned history retention：Workbench History / Pinned Results projection 现在会把仍保留在 session-local pinned store、但已被 bounded history/events 淘汰的 pinned entries 补回列表；Locations、Symbols、Workspace Outline 和 diagnostics 的 restore 路径也会在普通 history miss 时回退到 pinned entry。当前仍不持久化 pinned 状态，不跨 tab/project 共享，不改变 history 上限，event-backed auxiliary family 仍依赖已有 auxiliary snapshot 才能精确恢复。
- 阶段 423 修正阶段 7 的 LSP Workbench auxiliary pinned snapshot retention：event-backed auxiliary family 的 Workbench pin/unpin 现在会同步保留/释放对应 panel snapshot，`AttoLspWorkbenchAuxiliaryHistoryStore.entry(eventSequence:)` 在普通 bounded snapshot miss 时可回退到 pinned snapshot；Document Colors/Code Lens/Inlay Hints/Document Links/Hierarchy 等 pinned auxiliary results 被普通 auxiliary history 淘汰后仍可精确恢复。当前仍不持久化 snapshots，不跨 tab/project 共享，不改变 auxiliary history 上限，也不迁移到真正内嵌 dock。
- 阶段 424 维护阶段 7 的 LSP Workbench 测试结构：最近新增的 pinned-only history projection、pinned entry bounded retention 和 pinned auxiliary snapshot bounded retention 回归已从超大的 `AttoEditorCommandTests` 拆到独立 `AttoEditorLspWorkbenchTests`，后续 Workbench retention/history 回归有了更窄的落点。当前仅做测试结构拆分，不改变 runtime 行为，也尚未迁移旧类中全部 Workbench 测试。
- 阶段 425 继续维护阶段 7 的 LSP Workbench 测试结构：Workbench jump、selected refresh、selected clear stale、selected pin/unpin、unpin current、history summary/restore/filter 和 Document Colors refresh-only Workbench projection 等不依赖真实 window 的回归已迁入 `AttoEditorLspWorkbenchTests`；`AttoEditorCommandTests` 仍保留需要真实 AppKit child window、panel content view 或 `EditorCoreSkiaView` 的 Workbench UI 展示测试。当前仍只做测试结构拆分，不改变 runtime 行为，也尚未迁移所有真实 panel UI 回归。
- 阶段 426 维护阶段 8 的 command macro 测试结构：command macro 录制/回放、last/named macro 持久化、delete undo/history panel、history remove/clear、Sublime macro import/export 和 native file selection provider 回归已从超大的 `AttoEditorCommandTests` 拆到独立 `AttoEditorCommandMacroTests`；`AttoEditorCommandTests` 继续保留 command registry、菜单、palette 和 typed command argument 等相邻命令系统测试。`AttoAppDelegate` 测试 initializer 增加默认值不变的 session manager 注入点，方便 window 测试使用临时 session file；当前仅做测试结构/测试稳定性拆分，不改变默认 runtime 行为，也未迁移阶段 8 其余命令/菜单测试。
- 阶段 427 继续补齐阶段 9 的 schema-aware settings UI 基础：新增 `AttoConfigurationSettingsSchema` typed field schema，覆盖当前 editor/rendering/language/workspace settings key path、值类型、choice 和 scoped selector 支持元数据；workspace 字段明确不支持 `scoped_settings` 文档 selector。当前只提供 UI/校验/补全可消费的 schema descriptor，不改变 settings 解析/合并/保存语义，不实现表单 UI、runtime override 持久化或跨 schema 字段语义迁移。
- 阶段 428 继续补齐阶段 9 的 schema-aware settings UI 基础：新增 `AttoConfigurationSettingsValidation`，schema descriptor 现在能对 typed settings 产生 validation result/issue，覆盖 choice、wrap indent 语法、font/LSP 数值边界和 scoped selector 空值。当前只提供保存前/UI 可消费的校验模型，不改变 settings 加载/保存/合并语义，也不阻止 App 继续容忍现有无效 settings。
- 阶段 429 继续补齐阶段 9 的 schema-aware settings UI 基础：新增非破坏性的 settings file validation helper，并提供 `settings.validate_user_settings` / `settings.validate_workspace_settings` 命令和菜单入口，能对现有 user/workspace settings JSON 运行 typed schema validation 并显示 valid/invalid/missing/decode failure 状态。当前仍不是完整表单编辑器或补全 UI，不自动创建 settings 文件，也不触发 store 的损坏备份/迁移写盘路径。
- 阶段 430 继续补齐阶段 9 的 schema-aware settings UI 基础：新增 `AttoSettingsValidationPanelController`，settings validation 命令在 schema issue 存在时会打开可过滤的 issues panel，显示 key path、message、severity 和 scope；valid 结果会隐藏旧 panel。当前仍不实现完整表单编辑器、补全 UI、issue 跳转到 JSON 行列、自动修复、runtime override 持久化或跨 schema 字段语义迁移。
- 阶段 431 继续补齐阶段 9 的 schema-aware settings UI 基础：新增 settings JSON source location index，settings file validation report 会把 schema issue key path 映射到 1-based line/column，validation panel item 在有位置时显示该 source location。当前仍不实现完整表单编辑器、补全 UI、真实 editor jump/open action、自动修复、runtime override 持久化、完整 Sublime selector grammar 或跨 schema 字段语义迁移。
- 阶段 432 继续补齐阶段 9 的 schema-aware settings UI 基础：settings validation panel 现在支持 selected issue、键盘上下移动、Enter / double-click 打开有 source location 的 issue，并通过既有 settings 文件打开路径定位到对应 JSON line/column。当前仍不实现完整表单编辑器、补全 UI、自动修复、JSON parser 诊断恢复、runtime override 持久化、完整 Sublime selector grammar 或跨 schema 字段语义迁移。
- 阶段 433 继续补齐阶段 10 的 ABI/envelope 覆盖：MultiDocument project LSP start/stop/restart plan 与 lifecycle event snapshot 现在有统一 `project_lsp_lifecycle` envelope C ABI、feature descriptor 和 Swift typed wrapper，operation 覆盖 `start_plan` / `stop_plan` / `restart_plan` / `lifecycle_events`，错误也返回 `{ ok, operation, status, value, error, version }` envelope。旧裸 JSON API 保留兼容；当前仍不实现跨 host capability negotiation 策略或 App 层逐面板降级 UI。Swift 定向测试已编译到新 wrapper，但链接受 SwiftPM plugin staticlib 缓存缺少新 symbol 阻断；重建 plugin target 需要 `skia-bindings` 联网下载且提升权限被审批层拒绝。
- 阶段 434 继续补齐阶段 10 的 ABI/host capability negotiation 文档面：ABI v1 草案现在明确 runtime capability negotiation 流程，要求 host 在启用 workflow 前检查 ABI/feature mask、优先使用 envelope API、把 legacy raw JSON 作为兼容 fallback、对未知 descriptor/status 保持前向兼容并按层级 free 返回字符串；UI FFI README 也补齐 project LSP lifecycle envelope 的 operation、成功和错误示例。当前仍不新增 runtime capability negotiation 代码，不解决 SwiftPM plugin staticlib 缓存/联网重建阻碍，也不实现 App 层逐面板降级 UI。
- 阶段 435 继续补齐阶段 10 的 headless ABI/envelope 覆盖：headless LSP edit/helper 面现在有 `lsp_edit_helper_envelope` feature 和统一 envelope C ABI，覆盖 on-type formatting params、apply text edits、semantic tokens intervals、document highlights/inlay hints/document links/code lens/document symbols/diagnostics processing edits、completion item text edits 与 apply completion item；Swift `LSPBridge` 也新增同 schema wrapper。旧裸 JSON/null+last_error 或 bool+last_error API 保留兼容。当前仍不新增 UI FFI symbol，不切换 App 主路径，不覆盖 Sublime/Tree-sitter processor JSON result 面，也不解决 SwiftPM UI plugin staticlib 缓存/联网重建阻碍。
- 阶段 436 继续补齐阶段 10 的 UI FFI ABI/envelope 覆盖：per-`EditorUi` LSP derived-state apply helper 现在有 `lsp_derived_state_application_envelope` feature 和统一 envelope C ABI，覆盖 diagnostics、document symbols、document highlights、folding ranges、inlay hints、code lens 与 document links apply；Swift `EditorUI` 新增 typed wrapper，Atto runtime optional capability report 也能展示该能力，Rust 与 Swift 定向测试均已通过。旧 `c_int` + `last_error` apply API 保留兼容。当前仍不切换 App LSP derived-state apply 主路径，不覆盖 semantic tokens raw buffer apply，也不改变 payload schema。
- 阶段 437 继续补齐阶段 10 的 headless ABI/envelope 覆盖：Sublime / Tree-sitter processor 和 Tree-sitter indenter JSON result 面现在有 `processor_result_envelope` feature 和统一 envelope C ABI，覆盖 Sublime process/scope、Tree-sitter process/update-mode 和 indenter reindent line。Swift `SublimeProcessor` / `TreeSitterProcessor` 新增 raw/typed envelope wrapper，并复用 `EcfProcessorResultEnvelope` schema；Rust ABI 与 Swift 定向测试均已通过。旧裸 JSON/null+last_error API 保留兼容。当前仍不新增 UI FFI symbol，不切换 App syntax/Tree-sitter 主路径，不新增 Swift `TreeSitterIndenter` 类型，也不改变 processor payload schema。
- 阶段 438 继续补齐阶段 10 的 UI FFI ABI/envelope 覆盖：per-`EditorUi` LSP semantic tokens raw buffer apply 现在有 `lsp_semantic_tokens_application_envelope` feature 和统一 envelope C ABI，覆盖 `editor_core_ui_ffi_editor_ui_lsp_apply_semantic_tokens_envelope_json(...)` 成功/错误结果；Swift `EditorUI` 新增 raw/typed envelope wrapper，并复用 `EcuLspDerivedStateApplicationEnvelope` schema 额外解码 `value.dataLen`。Atto runtime optional capability report 也能展示该能力。旧 `c_int` + `last_error` apply API 保留兼容。当前仍不切换 App typed semantic tokens apply 主路径，不改变 semantic tokens payload/delta baseline schema。
- 阶段 439 进入阶段 12 的 workspace search 起点：`MultiDocumentEditorUi` 新增 core-backed workspace file search，可在 core workspace roots 下按现有 `CompiledSearch` 语义搜索本地 UTF-8 文件，支持 include/exclude glob、case/regex/whole-word 和 bounded results；UI FFI/Swift wrapper 新增 `multi_document_workspace_file_search` feature、raw/envelope ABI 与 typed result model。AttoEditor Find in Files 的 Folder/workspace scope 现在优先消费 core workspace file search，runtime 或搜索失败时继续回退到现有 Swift workspace file index。当前仍不实现 replace-in-files preview/apply、streaming result cursor、persistent project index、recent/project/session restore 下沉。
- 阶段 440 继续推进阶段 12 的 replace-in-files 起点：`MultiDocumentEditorUi` 新增 workspace file replacement WorkspaceEdit 生成器，可复用 core-backed workspace file search 匹配语义，在 workspace roots 内按 include/exclude glob、case/regex/whole-word 和 bounded results 生成 `{"workspaceEdit": ..., "applyMode": ...}` envelope；regex replacement 支持 capture expansion，生成的 LSP UTF-16 ranges 可直接进入既有 WorkspaceEdit transaction preview/apply/undo。UI FFI/Swift wrapper 新增 `multi_document_workspace_file_replacement` feature 和 raw C ABI，Atto runtime optional capability report 同步该能力。当前仍不实现 Find in Files 替换 UI、替换 preview panel 操作接线、streaming result cursor、persistent project index、recent/project/session restore 下沉。
- 阶段 441 继续推进阶段 12 的 Find in Files replace-in-files App 接线：Find in Files sidebar 现在有 replacement 输入和 Folder scope 的 Replace All 操作；runtime 支持 `multi_document_workspace_file_replacement` 时，AttoEditor 会通过 `MultiDocumentEditorUI.workspaceFileReplacementWorkspaceEditJSON(...)` 生成 atomic WorkspaceEdit，并复用既有 WorkspaceEdit transaction preview/apply/undo 路径。该阶段也修复本地 `file://` URI canonical path 等价匹配，避免 workspace file search 生成的 canonical URI 与 Swift open-tab URI 在 macOS `/var` / `/private/var` 上分歧时把打开 tab 当成未打开文件。当前仍不实现 Opened scope 替换、替换结果树/专用 preview 面板操作、search option UI 扩展、streaming result cursor、persistent project index、recent/project/session restore 下沉。
- 阶段 442 继续推进阶段 12 的 Find in Files 搜索选项 App 接线：Find in Files sidebar 现在暴露 Aa / Word / Regex 控件，并在窗口创建、偏好重应用、Opened scope core search、Folder scope core workspace search 和 Folder scope Replace All 生成 WorkspaceEdit 时统一传递 `SearchOptions` / `EcuSearchOptions`；Swift fallback 文件搜索也按同一组选项匹配。当前仍不实现 Opened scope 替换、替换结果树/专用 preview 面板操作、streaming result cursor、persistent project index、recent/project/session restore 下沉，也不新增自定义 word-boundary UI。
- 阶段 443 继续推进阶段 12 的 recent/session 迁移：`MultiDocumentEditorUi` 现在拥有 core recent file URI 列表，支持去重、MRU 排序、20 条上限、session restore 和 workspace roots 变化清理；UI FFI/Swift wrapper 新增 `multi_document_recent_files` feature、typed API 和 snapshot `recent_files` 字段；AttoEditor 的 remember/restore/session snapshot/Quick Open 优先消费 core recent state 并保留 Swift fallback。当前仍不实现 recent projects、project root persistence、persistent project index、跨窗口共享 recent store 或完整 session restore 下沉。
- 阶段 444 继续推进阶段 12 的 project file/Quick Open 迁移：`MultiDocumentEditorUi` 现在可基于 core workspace roots 枚举本地项目文件，输出 bounded、glob-filtered、稳定排序的 `uri/path/relative_path` 文件列表；UI FFI/Swift wrapper 新增 `multi_document_workspace_file_list` feature 和 typed API；AttoEditor Quick Open 与 Find in Files 的空查询项目文件枚举优先消费 core workspace file list，旧 runtime 或失败时继续回退 Swift `AttoWorkspaceFileIndex`。当前仍不实现 persistent project index、增量文件 watcher、recent projects、project root persistence 或完整 session restore 下沉。
- 阶段 445 继续推进阶段 12 的 session/project root persistence：Atto session window snapshot 新增可选 `workspaceRootURI`，保存时优先使用 core `MultiDocumentEditorUI.snapshot().workspaceRoots` 的首个本地 file URI，并继续写 legacy `workspaceRootPath`；恢复时统一通过 `AttoWindowSnapshot.validatedWorkspaceRootURL(...)` 优先校验 URI、再回退旧 path，旧 schema session 仍可读取。当前仍不实现 full core session snapshot/restore、unsaved buffer restore、recent projects、persistent project index 或跨窗口共享 project root history。
- 阶段 446 继续推进阶段 12 的 recent/project root 迁移：`MultiDocumentEditorUi` 现在拥有 core recent project/workspace root URI 列表，支持 trim、去重、MRU 排序和 20 条上限；UI FFI/Swift wrapper 新增 `multi_document_recent_projects` feature、typed API 和 snapshot `recent_projects` 字段；AttoEditor 在窗口 workspace root 创建/切换时同步 core recent projects，并提供内部读取 helper。当前仍不实现 recent-project UI、磁盘持久化项目列表、跨窗口共享 project root history、persistent project index、full core session snapshot/restore 或 unsaved buffer restore。
- 阶段 447 继续推进阶段 12 的 project index 迁移：`MultiDocumentEditorUi` 现在拥有 core-owned project file index snapshot，refresh 使用 workspace file list 的 root trust boundary、隐藏目录/`target`/`.build` 跳过、稳定排序和 bounded result 语义；UI FFI/Swift wrapper 新增 `multi_document_project_file_index` feature、refresh/snapshot/clear typed API；AttoEditor Quick Open / project file provider 优先刷新并读取 core index，失败或旧 runtime 时回退 core on-demand list / Swift `AttoWorkspaceFileIndex`。当前仍不实现增量 file watcher、磁盘持久化 index、跨窗口共享 project file cache、project index UI、full core session snapshot/restore 或 unsaved buffer restore。
- 阶段 448 继续推进阶段 12 的 project index 迁移：core-owned project file index 现在支持基于最后一次 refresh snapshot 的 fuzzy path query，UI FFI/Swift wrapper 新增 `multi_document_project_file_index_query` feature 和 typed query result；AttoEditor Quick Open 会在搜索文本变化时刷新命令并优先使用 core query，旧 runtime 或失败时继续回退 core list / Swift index 过滤。当前仍不实现增量 file watcher、磁盘持久化 index、跨窗口共享 project file cache、Quick Open UI 重设计、full core session snapshot/restore 或 unsaved buffer restore。
- 阶段 449 继续推进阶段 12 的 recent/session 迁移：Atto session snapshot 新增向后兼容的 `recentProjectURIs` 顶层字段，保存时从各窗口 core recent project store 聚合最近项目 URI，并在 session restore 后把仍存在的本地目录恢复到窗口 core recent projects store。当前仍不实现 recent-project UI、跨进程共享 recent project history、project file index 磁盘持久化、full core session snapshot/restore 或 unsaved buffer restore。
- 阶段 450 继续推进阶段 12 的 recent-project UI：AttoEditor 新增 `file.open_recent_project` command/menu 和 recent project quick panel，数据源消费窗口 core recent project store，按 active window 优先、目录存在性过滤和去重生成可执行项目列表，选择后创建并聚焦对应 workspace window。当前仍不实现持久 recent-project sidebar、跨进程共享 history、project file index 磁盘持久化、full core session snapshot/restore 或 unsaved buffer restore。
- 阶段 451 继续推进阶段 12 的 session restore / unsaved buffer handling：Atto session tab snapshot 新增向后兼容的 `isUntitled` / `unsavedText` 字段，只为 untitled buffer 持久化当前文本；恢复时 untitled tab 不再依赖磁盘文件存在，会重建内存 buffer、恢复文本并同步 dirty 状态到 core multi-document mirror。当前仍不持久化普通磁盘文件的未保存编辑、不实现 full core session snapshot/restore ABI、不新增跨窗口 unsaved buffer store，也不改变 Save/Save As、dirty close prompt 或 recent-project UI 行为。
- 阶段 452 继续补齐阶段 10 的 Swift future-compat 测试覆盖：`EcuLspSemanticTokensApplicationEnvelope` 现在有专门回归，覆盖未来 top-level/value/error metadata、未知 envelope status、`value.dataLen` 和未知 numeric error status 的解码兼容；shared derived-state application value decoder 也兼容默认 `data_len` 与 snake-case-converting `dataLen` 两种 host 解码策略。当前仍不改变 semantic tokens apply runtime 行为，也不切换 App typed semantic tokens apply 主路径。
- 阶段 453 继续补齐阶段 10 的 Swift future-compat 测试覆盖：`EcfProcessorResultEnvelope` 现在有专门回归，覆盖未来 processor operation/status/value metadata、未来 top-level/error metadata，以及未知 numeric error status 解码为 `nil`。当前仍不改变 Sublime/Tree-sitter processor runtime 行为，也不新增 Swift `TreeSitterIndenter` 类型。
- 阶段 454 继续补齐阶段 10 的 Swift runtime compatibility 诊断覆盖：headless/UI runtime compatibility report 现在有 load-error 回归，固定 runtime info 读取失败时 `isCompatible == false`、保留 required/optional feature 详情、用户可见诊断包含底层读取错误且不混入 missing feature 文案。当前仍不改变 Rust/C ABI、不新增 feature bit、不改 runtime probing API，也不实现完整第三方 host capability negotiation policy。
- 阶段 455 继续补齐阶段 10 的 runtime descriptor 对齐覆盖：headless/UI Swift smoke tests 现在会按 feature flag 验证 runtime JSON `features[]` 覆盖每个 Swift `knownFeatures` 项、descriptor bit 与 flag 一致，并要求 descriptor name/description 非空，降低新增 feature bit 时 host capability negotiation 诊断漂移风险。当前仍不改变 Rust/C ABI、不新增 feature bit、不改变 runtime JSON schema，也不实现完整第三方 host capability negotiation policy。
- 阶段 456 继续补齐阶段 10 的 runtime compatibility 组合诊断覆盖：headless/UI Swift runtime compatibility tests 现在固定 ABI 过旧、required feature 缺失和 optional feature 缺失同时发生时的 report 内容与用户可见诊断，避免 host 启动时只暴露单一失败原因。当前仍不改变 Rust/C ABI、不新增 feature bit、不改 runtime probing API，也不实现完整第三方 host capability negotiation policy。
- 阶段 457 继续补齐阶段 10 的 Swift capability snapshot 支持：headless/UI Swift wrapper 现在能通过 `runtimeCapabilitySnapshot()` 类型化解码 `runtime_info_json()`，暴露 runtime kind、ABI version、version、feature flags、feature descriptors、`runtimeInfo` 投影和 `supports(...)` helper，方便 Swift host 消费 descriptor 诊断信息而不用自行解析原始 JSON。当前仍不改变 Rust/C ABI、不新增 feature bit、不改变 runtime JSON schema、不切换现有 runtime compatibility 主路径，也不实现完整第三方 host capability negotiation policy。
- 阶段 458 继续补齐阶段 10 的 Swift capability negotiation 便利入口：headless/UI runtime compatibility evaluator 现在能直接消费 typed capability snapshot，并复用既有 `runtimeInfo` evaluator 逻辑；测试覆盖真实 runtime snapshot 兼容和构造的缺 feature snapshot。当前仍不改变 Rust/C ABI、不新增 feature bit、不改变 runtime JSON schema、不改变 `evaluate(library:)` 行为，也不实现完整第三方 host capability negotiation policy。
- 阶段 355 进一步补齐阶段 5 的 core document URI projection：`file.reload` command/menu 和 active-tab reload helper 已使用 core tab snapshot 的 `document_uri` 投影读取磁盘内容，并同步 AppKit 文本、core tab text 与 dirty/saved 状态；该完成项属于 `PLAN.md` 阶段 5 的 `feat(app): reload active tab from core uris` 增量，`SWIFT-GAPS.md` 仅作为任务范围参考。
- 阶段 356 进一步补齐阶段 5 的 core document URI projection：active-tab save 主路径已使用 core tab snapshot 的 `document_uri` 投影作为真实写盘 URL，并同步 core tab text、dirty/saved 状态、document URI/title、LSP didSave 和 save callback；untitled Save Panel 仍显式使用用户选择 URL。该完成项属于 `PLAN.md` 阶段 5 的 `feat(app): save active tab from core uris` 增量，`SWIFT-GAPS.md` 仅作为任务范围参考。
- 阶段 357 进一步补齐阶段 5 的 core tab preview/pin projection：`file.pin_tab` command/menu 和 active-tab pin helper 已使用 core snapshot 的 active tab / `is_preview` 投影来 pin preview tab；该完成项属于 `PLAN.md` 阶段 5 的 `feat(app): pin active tab from core tabs` 增量，`SWIFT-GAPS.md` 仅作为任务范围参考。
- 阶段 358 进一步补齐阶段 5 的 core pane active-view projection：pane focus/move/close 用户路径已在操作前消费 core snapshot 的 `active_view_index` 投影来确定当前 pane；该完成项属于 `PLAN.md` 阶段 5 的 `feat(app): project active panes from core views` 增量，`SWIFT-GAPS.md` 仅作为任务范围参考。
- 阶段 359 进一步补齐阶段 5 的 core tab order projection：tab bar drag reorder 和 move-tab command 已使用 core snapshot 的 tab order / active tab 投影来计算移动来源和目标；该完成项属于 `PLAN.md` 阶段 5 的 `feat(app): drag tabs with core tab order` 增量，`SWIFT-GAPS.md` 仅作为任务范围参考。
- 阶段 360 进一步补齐阶段 5 的 dirty close document URI projection：dirty tab close confirmation 已使用 core tab snapshot 的 `document_uri` 投影作为用户可见文件名；该完成项属于 `PLAN.md` 阶段 5 的 `feat(app): project dirty close prompts from core uris` 增量，`SWIFT-GAPS.md` 仅作为任务范围参考。
- 阶段 361 进一步补齐阶段 5 的 core preview tab projection：打开新的 preview 文件和把已有 preview tab 重新打开为 pinned 的路径已使用 core snapshot 的 `is_preview` / tab order 投影；该完成项属于 `PLAN.md` 阶段 5 的 `feat(app): replace preview tabs from core state` 增量，`SWIFT-GAPS.md` 仅作为任务范围参考。
- 阶段 362 进一步补齐阶段 5 的 preview replacement close callback projection：preview tab replacement 隐式关闭旧 preview tab 时，`onDidCloseFile` callback 已使用替换前 core tab snapshot 的 `document_uri` 投影；该完成项属于 `PLAN.md` 阶段 5 的 `feat(app): project preview close callbacks from core uris` 增量，`SWIFT-GAPS.md` 仅作为任务范围参考。
- 阶段 363 进一步补齐阶段 5 的 core tab order projection：Save All / close-window dirty Save All 路径已按 core snapshot 的 tab order 遍历 dirty tabs；该完成项属于 `PLAN.md` 阶段 5 的 `feat(app): save dirty tabs in core order` 增量，`SWIFT-GAPS.md` 仅作为任务范围参考。
- 阶段 364 进一步补齐阶段 5 的 close active fallback projection：关闭当前 tab 后，下一选中 tab 已使用 core `MultiDocumentEditorUi.close_tab` 后的 active tab fallback 投影，而不是 Swift 本地 `tabs.last`；该完成项属于 `PLAN.md` 阶段 5 的 `feat(app): project close active fallback from core tabs` 增量，`SWIFT-GAPS.md` 仅作为任务范围参考。
...
- 阶段 382 继续补齐阶段 6 的 core-owned project LSP typed lifecycle plan 起点：`MultiDocumentEditorUi` 已能用 open tab document URI + language id + project LSP server configs 生成 auto-start plan，并通过 UI FFI/Swift typed wrapper 暴露。当前仍不改变实际 LSP server 启停路径，剩余缺口是把该 plan 接到 typed lifecycle 执行、shared-session ownership 和跨 project 策略。
- 阶段 383 继续补齐阶段 6 的 core start-plan App 消费：AttoEditor 的 project/root 自动启动路径现在消费 `MultiDocumentEditorUI.projectLspStartPlan()`，不再只靠 Swift 本地遍历 open tabs 后直接启动；core plan 为空或候选与 plan 的 document/server/tab 不匹配时不会启动。当前仍通过 Swift `EditorUI.lspEnable(...)` 执行启动，剩余缺口是把执行、ownership、restart/stop 和 shared-session 策略继续下沉。
- 阶段 384 继续补齐阶段 6 的 project LSP lifecycle outcome 记录：core `MultiDocumentEditorUi` 现在有 project LSP start outcome event stream，Swift/App 在自动启动成功或失败后会记录 started/failed outcome；剩余缺口是把真正启动执行、进程 ownership、restart/stop、恢复策略和 shared-session/project 策略继续下沉。
- 阶段 385 继续补齐阶段 6 的 project LSP lifecycle outcome UI 消费：Project Status Events panel 和 Project Dashboard 现在会消费 core start outcome stream，展示 lifecycle events 计数和 started/failed 明细；剩余缺口仍是把真正启动执行、进程 ownership、restart/stop、恢复策略和 shared-session/project 策略继续下沉。
- 阶段 386 继续补齐阶段 6 的 project LSP restart outcome 记录：active-tab manual restart、project restart 和 auto-restart 现在也会写入 core lifecycle outcome stream，并以 `operation = "restart"`、对应 trigger 和 started/failed status 表达；剩余缺口仍是把真正 restart/stop 执行、进程 ownership、恢复策略和 shared-session/project 策略继续下沉。
- 阶段 387 继续补齐阶段 6 的 project LSP stop outcome 记录：用户关闭拥有 LSP session 的 tab 或显式切换到非 LSP 语言时，现在也会写入 core lifecycle outcome stream，并以 `operation = "stop"`、`trigger = "tab_close"` / `"language_change"` 和 `status = "stopped"` 表达；剩余缺口仍是把真正启停/restart 执行、进程 ownership、恢复策略和 shared-session/project 策略继续下沉。
- 阶段 388 继续补齐阶段 6 的 host-visible LSP shutdown App 入口：active-tab manual shutdown 现在也会调用 Swift UI wrapper 的 `EditorUI.lspShutdown()`，并以 `operation = "stop"`、`trigger = "manual_shutdown"` 和 `status = "stopped"` 写入 core lifecycle outcome stream；剩余缺口仍是把真正启停/restart 执行、进程 ownership、恢复策略和 shared-session/project 策略继续下沉。
- 阶段 389 继续补齐阶段 6 的 project-level LSP shutdown App 入口：project-level manual shutdown 现在会按 core-projected open tabs 调用 Swift UI wrapper 的 `EditorUI.lspShutdown()`，并以 `operation = "stop"`、`trigger = "project_shutdown"` 和 `status = "stopped"` 写入 core lifecycle outcome stream；剩余缺口仍是把真正启停/restart 执行、进程 ownership、恢复策略和 shared-session/project 策略继续下沉。
- 阶段 390 继续补齐阶段 6 的 core-owned project LSP stop/shutdown plan 查询面：project-level manual shutdown 现在先消费 `MultiDocumentEditorUI.projectLspStopPlan()`，再执行 Swift wrapper 的 `EditorUI.lspShutdown()`；剩余缺口仍是把真正启停/restart 执行、进程 ownership、恢复策略和 shared-session/project 策略继续下沉。
- 阶段 391 继续补齐阶段 6 的 core-owned project LSP restart plan 查询面：project-level manual restart 现在先消费 `MultiDocumentEditorUI.projectLspRestartPlan()`，再执行 Swift wrapper 的 `EditorUI.lspDisable()` / `EditorUI.lspEnable(...)` restart pipeline；剩余缺口仍是把真正启停/restart 执行、进程 ownership、恢复策略和 shared-session/project 策略继续下沉。
- 阶段 392 继续补齐阶段 6 的 core restart-plan App 消费覆盖面：failed/exited 自动恢复现在也会先消费 `MultiDocumentEditorUI.projectLspRestartPlan()`，再执行 Swift wrapper 的 `EditorUI.lspDisable()` / `EditorUI.lspEnable(...)` restart pipeline；剩余缺口仍是把真正启停/restart 执行、进程 ownership、恢复策略和 shared-session/project 策略继续下沉。
- 阶段 393 继续补齐阶段 6 的 core restart-plan App 消费覆盖面：active-tab manual restart 现在也会先消费 `MultiDocumentEditorUI.projectLspRestartPlan()`，再执行 Swift wrapper 的 `EditorUI.lspDisable()` / `EditorUI.lspEnable(...)` restart pipeline；剩余缺口仍是把真正启停/restart 执行、进程 ownership、恢复策略和 shared-session/project 策略继续下沉。
- 阶段 394 继续补齐阶段 6 的 core stop-plan App 消费覆盖面：active-tab manual shutdown 现在也会先消费 `MultiDocumentEditorUI.projectLspStopPlan()`，再执行 Swift wrapper 的 `EditorUI.lspShutdown()` graceful shutdown；剩余缺口仍是把真正启停/restart 执行、进程 ownership、恢复策略和 shared-session/project 策略继续下沉。
- 阶段 395 继续补齐阶段 6 的 core plan gate lifecycle 可见性：active-tab manual restart/shutdown 被 core restart/stop plan gate 拒绝时会写入 `status = "skipped"` 的 restart/stop lifecycle outcome；剩余缺口仍是把真正启停/restart 执行、进程 ownership、恢复策略和 shared-session/project 策略继续下沉。
- 阶段 396 继续补齐阶段 6 的 core lifecycle 执行状态：active-tab manual restart/shutdown 通过 core restart/stop plan gate 后会先写入 `status = "requested"` 的 restart/stop lifecycle event，再执行 Swift wrapper 并写入终态 outcome；剩余缺口仍是把真正启停/restart 执行、进程 ownership、恢复策略和 shared-session/project 策略继续下沉。
- 阶段 397 继续补齐阶段 6 的 core lifecycle attempt correlation：active-tab manual restart/shutdown 的 requested 和终态 outcome 现在通过同一个 `attempt_id` 关联；剩余缺口仍是把真正启停/restart 执行、进程 ownership、恢复策略和 shared-session/project 策略继续下沉。
...
- 阶段 366 继续推进阶段 9 的 settings scopes App 接线：AttoEditor 打开文档、split pane 和偏好重应用路径现在会按文档 URL + language id 解析 scoped settings snapshot，同一 window 内不同 tab 可使用不同 font/wrap/search/language gate 等 editor/language 配置，active tab Find bar、format-on-save 和 semantic highlighting gate 也会跟随 active document scoped config；该完成项属于 `PLAN.md` 阶段 9 的 `feat(app): apply scoped settings to documents` 增量，`SWIFT-GAPS.md` 仅作为任务范围参考。
- 阶段 367 继续推进阶段 9 的 scoped rendering 接线：AttoEditor 现在会把 scoped settings 中的 `rendering.theme_name` 应用到对应 document/tab 的 Skia theme，新建 tab、split pane、已打开 tab preference reapply、window-wide theme reapply 和 active tab 内容背景都会随 active document selector 解析结果切换；该完成项属于 `PLAN.md` 阶段 9 的 `feat(app): switch scoped tab themes` 增量，`SWIFT-GAPS.md` 仅作为任务范围参考。
- 阶段 368 继续推进阶段 9 的 settings 编辑入口：AttoEditor 现在提供 `settings.open_user_settings` 和 `settings.open_workspace_settings` 命令/菜单入口，可打开或创建 user settings 与当前 workspace `.attoeditor/settings.json`；缺失文件会写入非侵入式 JSON scaffold，实际配置保持空，`scoped_settings` 示例放在 decoder 忽略的 `_examples` 字段，编辑仍复用现有 tab/core mirror 路径。该完成项属于 `PLAN.md` 阶段 9 的 `feat(app): open scoped settings files` 增量，`SWIFT-GAPS.md` 仅作为任务范围参考。仍缺完整 Sublime settings selector grammar、schema-aware settings 表单/校验/补全 UI、runtime override UI/持久化、跨 schema 字段语义迁移和完整 capability negotiation。
- 阶段 369 继续推进阶段 9 的自定义 word-boundary 配置接线：AttoEditor 现在支持在 `AttoConfigurationSnapshot` / `AttoConfigurationSettings` / `AttoPreferences` 中配置 `word_boundary_ascii_boundary_chars`，并在创建 tab、split pane 和偏好重应用时通过 Swift FFI 应用到 editor；Rust whole-word search/find/replace/search-highlight 与 multi-document search 会消费同一份 `WordBoundaryConfig`，字段缺省时显式 reset 到 core 默认边界。该完成项属于 `PLAN.md` 阶段 9 的 `feat(app): configure search word boundaries` 增量，`SWIFT-GAPS.md` 仅作为任务范围参考。仍缺完整 Sublime settings selector grammar、schema-aware settings 表单/校验/补全 UI、runtime override UI/持久化、跨 schema 字段语义迁移和完整 capability negotiation。
...
- 阶段 6 后续缺口中，基础持久在线 references/locations panel 已在阶段 114 补齐，locations/symbols result lifecycle store 起点已在阶段 121 补齐，locations/symbols history entry/envelope 元数据起点已在阶段 122 补齐；多数 result family typed payload 已在阶段 148-161 和阶段 173-174 补齐；阶段 247 已补单 root LSP `workspaceFolders` initialize / client response 起点，阶段 248 已补手动 `workspace/didChangeWorkspaceFolders` 通知与 response 列表更新链路，阶段 249 已补 core-owned root diff 到 AttoEditor active LSP didChange 的接线，阶段 250 已把 root change fan-out 扩展到所有 open-tab LSP sessions，阶段 251 已补保存/关闭路径的 `textDocument/didSave` / `textDocument/didClose` 通知，阶段 252 已补打开路径的 `textDocument/didOpen` 通知，阶段 253 已补 extra document 的 `textDocument/didChange` 通知，阶段 254 已补 shared-session root change 重复通知去重，阶段 255 已补 status snapshot 到 Swift/App status bar 的 workspace root 与 capability 可观测性，阶段 256 已补 LSP status changed state event 起点，阶段 257 已让 AttoEditor status bar 消费 active `lsp_status_changed` event payload，阶段 258 已扩大低层 sync/on-type/result apply failure 的 status event 覆盖，阶段 259 已让 project-level lifecycle drain 消费 MultiDocument LSP status failure event，阶段 260 已提供 project LSP status events 轻量面板入口，阶段 261 已把 server progress/activity 当前状态变化纳入去重后的 `lsp_status_changed`，阶段 262 已把 server process running/exited health 纳入 status snapshot 和事件流，阶段 263 已提供 active-tab manual server restart 起点，阶段 264 已提供按 core workspace tab 投影执行的 project-level manual restart 起点，阶段 265 已补关闭 owned LSP tab 时释放 handle 且不重复 didClose 的停止侧起点，阶段 266 已补最后一个 shared LSP handle drop 时调用 graceful exit 的 Rust 起点，阶段 267 已补 Rust/C ABI/Swift wrapper 的显式 LSP shutdown API，阶段 268 已补 workspace folder didChange 后 shared-session root alias/pool 更新，阶段 269 已补 root 变更/session restore 后的 App 侧批量 LSP 启动起点，阶段 297/298 已补 core-owned project LSP launch metadata 和 open tab language metadata，阶段 382 已补 core-owned project LSP auto-start plan 查询面，阶段 383 已让 App project/root 自动启动路径消费该 plan，阶段 384 已补 core-owned project LSP start outcome event stream 并让 App 自动启动写回 started/failed 结果，阶段 385 已让 Project Status Events panel / Project Dashboard 消费并展示这些 lifecycle outcomes，阶段 386 已把 active/project/auto restart 的 started/failed outcome 也写入同一 core lifecycle stream，阶段 387 已把用户级 tab close / language change stop 的 stopped outcome 写入同一 stream，阶段 388 已把 active-tab manual shutdown 入口接到 `EditorUI.lspShutdown()` 并写入 `manual_shutdown` stop outcome，阶段 389 已把 project-level manual shutdown 入口接到 core-projected open tabs 并写入 `project_shutdown` stop outcome，阶段 390 已补 core-owned project LSP stop/shutdown plan 查询面并让 App project shutdown 消费该 plan，阶段 391 已补 core-owned project LSP restart plan 查询面并让 App project restart 消费该 plan，阶段 392 已让 failed/exited auto-restart 消费同一个 restart plan，阶段 393 已让 active-tab manual restart 消费同一个 restart plan，阶段 394 已让 active-tab manual shutdown 消费同一个 stop plan，阶段 395 已让 active-tab manual restart/shutdown 的 restart/stop plan gate 拒绝写入 skipped lifecycle outcome，阶段 396 已让 active-tab manual restart/shutdown 通过 plan gate 后先写入 requested lifecycle event，阶段 397 已让 requested 与终态 outcome 通过 `attempt_id` 稳定关联；阶段 6 的 document open/change/save/close control-plane 链路已有基础覆盖，status bar 已能展示 server 状态、workspace root、activity、process health 和 compact capabilities，Swift/App 可从统一 state event stream 订阅并消费更多 LSP status 变化，core/App 也已有 project LSP auto-start/restart/stop plan 的 typed 查询/消费、auto-start/restart/user stop/active-tab manual shutdown/project shutdown/plan-gate skipped/requested execution outcome 记录、attempt correlation 和 dashboard 可见性起点。仍缺把实际启动/restart/stop/shutdown 执行与进程 ownership 真正下沉到 core、更完整 shared-session/project ownership 策略、少数 raw-only family typed envelope、更完整 progress/activity 历史模型、完整 dashboard 级 project status panel 和项目级命令模型。
...
- 阶段 10 后续缺口中，rich documentation/detail preview 已在阶段 21 补齐，commitCharacters 提交行为已在阶段 22 补齐，server triggerCharacters 自动触发已在阶段 23 补齐，增量过滤已在阶段 24 补齐，跨文件 WorkspaceEdit 摘要预览已在阶段 25 补齐，打开 tab / 本地 `file://` 文档 text edits 应用已在阶段 47 补齐，本地未打开文件的 resource operations 已在阶段 55 补齐，打开 tab 相关 resource operations 已在阶段 59 补齐，阶段 196-221 已补 core transaction 起点、root-gated 本地文件覆盖、App apply helper 主路径、基础 preview/confirmation、专用 diff preview panel 起点和未打开文件 text edit rollback、open-tab rollback、atomic apply mode、atomic runtime failure rollback、resource-order dependency preflight、ordered unsupported dependency preflight、resource operation typed summary、dirty/conflict typed summary、打开 tab undo grouping 起点、最近一次 transaction undo 起点、用户级 command/menu/keymap 起点、专门 history panel 起点、bounded 多级 transaction-wide undo stack 和 redo；仍缺更深层 conflict 检测/解决 UI 和统一 feedback/状态订阅模型。
...
- 阶段 11 后续缺口中，跨文件 WorkspaceEdit 摘要预览已在阶段 25 补齐，code action diagnostics context 已在阶段 26 补齐，code action kind/filter 产品化已在阶段 27 补齐，打开 tab / 本地 `file://` 文档 text edits 应用已在阶段 47 补齐，code action command payload 执行结果/错误展示已在阶段 48 补齐，本地未打开文件的 resource operations 已在阶段 55 补齐，打开 tab 相关 resource operations 已在阶段 59 补齐，code action result/resolve typed payload wrapper 和 App 主路径消费已在阶段 152 补齐，阶段 196-221 已补 core WorkspaceEdit transaction 起点、root-gated 本地文件覆盖、App apply helper 主路径、基础 preview/confirmation、专用 diff preview panel 起点和未打开文件 text edit rollback、open-tab rollback、atomic apply mode、atomic runtime failure rollback、resource-order dependency preflight、ordered unsupported dependency preflight、resource operation typed summary、dirty/conflict typed summary、打开 tab undo grouping 起点、最近一次 transaction undo 起点、用户级 command/menu/keymap 起点、专门 history panel 起点和 bounded 多级 transaction-wide undo stack、redo 已完成；仍缺更深层 conflict 检测/解决 UI。
...
- 阶段 13 后续缺口中，按 server trigger characters 自动触发已在阶段 14 补齐，显式 formatting 错误展示和 typed outcome 已在阶段 60 补齐；显式 Format Document / Format Selection 的 transaction apply/request retry owner 已在阶段 377 补齐。format-on-save 和 on-type 仍保留 blocking `TextEdit` apply helper。
...
- 阶段 14 后续缺口中，显式 formatting 错误展示和 formatting typed outcome 已在阶段 60 补齐；显式 Format Document / Format Selection 的 transaction apply/request retry owner 已在阶段 377 补齐。format-on-save 和 on-type 仍保留 blocking `TextEdit` apply helper。
...
阶段 208 仍不是 `PLAN.md` 阶段 4 终态：当前 AppKit 投影使用全量文本刷新，不等同于打开 tab undo grouping；preview/confirmation UI、text edits 与打开 tab 状态变更的完整 batch atomic rollback，以及更深层 conflict 检测/解决 UI 仍需后续阶段补齐。阶段 209 已继续补齐基础 preview/confirmation 起点。
...
测试覆盖 preview display model、单文档免确认、resource operation preview 文案、skipped detail 文案，以及 App 层取消确认时编辑器文本、磁盘文件和 core transaction event cursor 均不变化。本阶段当时仍不是完整 preview panel 产品化：阶段 210 已继续把基础确认 alert 推进为专用可导航 diff preview panel 起点；打开 tab undo grouping、完整 batch atomic rollback 和更深层 conflict 检测/解决 UI 仍需后续阶段补齐。
...
测试覆盖 text edit diff section、resource operation section，以及 App 取消确认路径中捕获的 preview 已包含按文件 diff sections。本阶段仍不是 `PLAN.md` 阶段 4 终态：panel 还不是可筛选/可展开树形 diff，也没有把打开 tab undo grouping、完整 batch atomic rollback 和更深层 conflict 检测/解决 UI 一起补齐。
...
本阶段当时仍不是 `PLAN.md` 阶段 4 终态：打开 tab 的文本修改和 tab/resource 状态变更仍没有统一 rollback/undo grouping；阶段 212 已继续补齐 fatal failure 下的打开 tab 状态 rollback，阶段 213 已继续补齐显式 atomic apply mode；完整 undo grouping 和更深层 conflict 检测/解决 UI 仍需后续阶段补齐。
...
本阶段仍不是 `PLAN.md` 阶段 4 终态：它只覆盖打开 tab 的 App 投影 undo group，不实现跨文件全局 undo command，不为未打开本地文件 text edit 或 resource operation 提供用户级 undo，也不完成更深层 conflict 检测/解决 UI。
...
本阶段仍不是 `PLAN.md` 阶段 4 终态：它不改变默认 partial apply 语义，不新增 ABI 函数，不提供跨文件用户级 undo command，也不完成更深层 conflict 检测/解决 UI。
...
测试覆盖 feature flag、runtime optional feature 报告、command registry metadata/schema、runtime gate、菜单 key equivalent，以及通过 `AttoAppDelegate.executeCommand(id: "workspace.undo_last_workspace_edit")` 恢复打开 tab 文本、窗口 dirty 状态和未打开文件内容。本阶段当时仍不是多级/global transaction undo 终态：命令只消费最近一次 core undo record，不接入 AppKit `UndoManager` 多级栈，不提供 redo，也不解决跨 transaction conflict resolution；当时剩余缺口收窄为更完整 conflict UI 和多级/global transaction-wide undo/redo 语义，后续阶段 345-347 已继续补齐多级 undo、AppKit undo aggregation 和 redo。
...
测试覆盖 Swift 本地 `tab.fileURL` 仍为旧路径、core `document_uri` 已改为新路径时，WorkspaceEdit changes 指向新 URI 会命中打开 tab 并更新 in-memory editor text，而不是修改 projected 文件磁盘内容。本阶段仍不实现 project root ownership、pane layout tree、drag/drop split、多级/global transaction-wide undo 或更深层 conflict 检测/解决 UI。
...
测试覆盖 Swift 本地 `tab.fileURL` 仍为旧路径、core `document_uri` 已改为新路径时，rename result WorkspaceEdit 指向 projected URI 会命中打开 tab 的 in-memory editor text，而不是修改 projected 文件磁盘内容。本阶段仍不实现 project root ownership、pane layout tree、drag/drop split、完整 project/LSP lifecycle 迁移、更深层 conflict 检测/解决 UI 或多级/global transaction-wide undo 语义。
...
## 阶段 345: WorkspaceEdit 多级 transaction undo stack

2026-08-03 阶段 345 已把 WorkspaceEdit transaction undo 从“最近一次一次性 record”推进到 bounded 多级 stack。`MultiDocumentEditorUi` 现在保留最多 64 条成功 WorkspaceEdit transaction 的 undo record，`undo_last_workspace_edit_transaction` 按后进先出顺序消费；失败或 rejected transaction 不再丢弃已有可撤销记录，超过上限时会丢弃最旧 record 并 best-effort 清理对应 filesystem backup。

该阶段保持既有 Rust/C ABI/Swift wrapper 形状不变：Swift `MultiDocumentEditorUI.undoLastWorkspaceEditTransaction()` 继续调用同一入口，但可连续撤销多条 transaction。AttoEditor 的 `workspace.undo_last_workspace_edit` command/menu/keymap 现在可重复执行；WorkspaceEdit History panel 不维护独立历史模型，而是继续消费 core transaction event stream，并用已消费 sequence 集合标出当前最新可撤销 apply event。

测试覆盖 Rust core、Swift UIFFI wrapper 和 AttoEditor App command/history panel：连续两个 WorkspaceEdit transaction 会按 LIFO 顺序恢复打开 tab 与未打开本地文件，history panel 的 `Undo Latest` 状态会随每次 undo 移动到上一条未消费 applied event。本阶段仍不实现 redo、AppKit `UndoManager` 聚合、跨 transaction conflict resolution 或更深层 conflict 检测/解决 UI。

## 阶段 346: WorkspaceEdit AppKit UndoManager aggregation 起点

2026-08-03 阶段 346 已把成功应用的 core-owned WorkspaceEdit transaction 注册到窗口 AppKit `UndoManager`。AttoEditor App 在 `MultiDocumentEditorUI.applyWorkspaceEditTransaction(...)` 成功后注册名为 `Workspace Edit` 的 undo action；该 action 复用既有 `undoLastCoreWorkspaceEditTransaction()`，因此仍由 core transaction undo stack 恢复打开 tab、未打开本地文件和 AppKit 投影。

该阶段不在 Swift 侧维护独立 WorkspaceEdit undo 历史，也不改变 `workspace.undo_last_workspace_edit` command/menu/keymap、Rust/C ABI 或 Swift wrapper。测试覆盖 `window.undoManager.undo()` 触发 WorkspaceEdit transaction undo 后，打开 tab 文本、未打开文件内容和窗口 dirty 状态恢复。

本阶段仍不实现 redo、跨 transaction conflict resolution 或更深层 conflict 检测/解决 UI；普通 editor buffer undo/redo 主路径也保持原有 `EditorUI.undo()` / `EditorUI.redo()` 语义。

## 阶段 347: WorkspaceEdit transaction redo

2026-08-03 阶段 347 已把 core-owned WorkspaceEdit transaction redo 从缺口推进到 Rust core、C ABI、Swift typed wrapper 和 AttoEditor App 主路径。`MultiDocumentEditorUi` 现在在 successful undo 后保留 redo record，`redo_last_workspace_edit_transaction` 会按 LIFO redo 顺序重新走 core WorkspaceEdit apply 路径；redo record 会移除 redo 用 `TextDocumentEdit.version`，避免 undo 后 core document version 增长导致同一 transaction 被 stale version 误拒绝。该完成项属于 `PLAN.md` 阶段 4 的 `feat(ui): redo workspace edit transactions` 增量；`SWIFT-GAPS.md` 仅作为任务范围参考，提交归属以 `PLAN.md` 对应章节为准。

Swift/UI 层新增 `ECU_FEATURE_MULTI_DOCUMENT_WORKSPACE_EDIT_TRANSACTION_REDO`、`MultiDocumentEditorUI.redoLastWorkspaceEditTransaction()`、workspace edit transaction envelope 的 `redo` operation、`workspace.redo_last_workspace_edit` command、Edit 菜单项、`cmd+option+shift+z` 默认 binding、AppKit `UndoManager` redo action 和 WorkspaceEdit History panel 的 redo event 展示。redo 后仍由 core snapshot 投影回打开 tab、未打开本地文件和窗口 dirty 状态；普通 editor buffer undo/redo 主路径保持原有 `EditorUI.undo()` / `EditorUI.redo()` 语义。

测试覆盖 Rust core、C ABI、Swift UIFFI wrapper、runtime feature descriptor、AttoEditor command/menu/keymap、AppKit `UndoManager` 和 history panel：打开 tab 与未打开文件可在 undo 后 redo，连续 undo/redo 保持顺序，redo event 会成为下一条可 undo transaction。本阶段仍不实现跨 transaction conflict resolution 或更深层 conflict 检测/解决 UI。

## 阶段 348: WorkspaceEdit conflict preview 分组

2026-08-03 阶段 348 已把 AttoEditor WorkspaceEdit preview panel 的 typed conflict 展示从逐条平铺推进到按 conflict kind/operation 分组。`AttoWorkspaceEditPreview` 现在保留 core transaction `apply_mode`，暴露 `conflictGroups` 和 `applyButtonTitle`；preview summary 会在有 conflict 时明确说明 partial apply 会跳过 conflict，atomic apply 会被 conflict 阻断。该完成项属于 `PLAN.md` 阶段 4 的 `feat(app): group workspace edit conflict previews` 增量；`SWIFT-GAPS.md` 仅作为任务范围参考，提交归属以 `PLAN.md` 对应章节为准。

Swift/App 层新增 human-readable conflict category、按 kind 的 suggested action，以及 conflict detail section 的 Category/Impact/Suggested action 字段；preview panel 的确认按钮在 partial conflict 场景下显示为 `Apply Non-Conflicting Changes`，避免用户误以为会全量应用。测试覆盖单一 dirty document conflict、多 kind conflict 分组、按钮文案、summary 文案和 detail section。该阶段不改变 Rust/FFI ABI、不改变 core transaction 语义，也不实现跨 transaction conflict resolution、自动修复或更深层 core conflict 检测。

## 阶段 349: WorkspaceEdit conflict impact schema

2026-08-03 阶段 349 已把 core-owned WorkspaceEdit transaction conflict 从 `kind/reason/operation/message` 扩展为包含 `severity`、`apply_impact` 和 `resolution` 的兼容 JSON schema。`apply_impact` 由 core 根据 transaction `apply_mode` 生成：partial conflict 为 `skips_change`，atomic conflict 为 `blocks_atomic_apply`；`resolution` 提供机器可读的解决动作类别，例如 `save_or_discard`、`refresh_request`、`restore_resource` 和 `unsupported`。该完成项属于 `PLAN.md` 阶段 4 的 `feat(ui): expose workspace edit conflict impact` 增量；`SWIFT-GAPS.md` 仅作为任务范围参考，提交归属以 `PLAN.md` 对应章节为准。

Swift UIFFI wrapper 已 typed decode 新字段，并为旧 JSON 保留 `warning` / `skips_change` / `inspect` 默认值。AttoEditor preview detail 现在显示 core-provided Severity 与 Apply impact，并按 `resolution` 生成 Suggested action；当 core 标记 `blocks_atomic_apply` 时，preview model `canApply` 为 false，preview panel 会禁用 Apply 按钮并显示 `Resolve Conflicts First`。该阶段不新增 C ABI 函数，不改变 WorkspaceEdit transaction apply/preview/undo/redo 语义，也不实现自动修复、跨 transaction conflict resolution 或 project-level conflict owner。

## 阶段 350: WorkspaceEdit blocking conflict App gate

2026-08-03 阶段 350 已把 AttoEditor App 的 WorkspaceEdit confirmation 路径补齐为真正消费 `AttoWorkspaceEditPreview.canApply`：当 core preview 标记 atomic blocking conflict 时，`confirmWorkspaceEditPreview(...)` 会在调用 core apply transaction 前直接返回 false，即使 testing decision provider 返回 `.apply` 或当前没有窗口也不能绕过。该完成项属于 `PLAN.md` 阶段 4 的 `feat(app): gate blocking workspace edit conflicts` 增量；`SWIFT-GAPS.md` 仅作为任务范围参考，提交归属以 `PLAN.md` 对应章节为准。

本阶段同时让 Swift `AttoWorkspaceEditParser` 解开 core transaction envelope 中的嵌套 `workspaceEdit`，使 App 入口可接受 `{"applyMode":"atomic","workspaceEdit":{...}}` 这类与 `MultiDocumentEditorUI` transaction API 一致的 payload。测试覆盖 envelope parser，以及 dirty open-tab delete 造成 atomic blocking conflict 时，App 只展示/捕获 preview、不推进 WorkspaceEdit transaction event、不修改未打开文件，并保留 `Resolve WorkspaceEdit conflicts before applying` 状态反馈。该阶段不新增 Rust/FFI ABI，不改变 core transaction 语义，也不实现自动解决 conflict、跨 transaction conflict resolution 或 project-level conflict owner。

## 阶段 351: WorkspaceEdit open-conflict resolution UI 起点

2026-08-03 阶段 351 已把 WorkspaceEdit conflict preview 从只显示 suggested action 推进到一个可执行 UI 起点：`AttoWorkspaceEditPreviewPanelController` 新增 `Open Conflict` 按钮和稳定 accessibility identifier，按钮会基于当前选中 section 或第一个 conflict 找到目标 URI，返回 `AttoWorkspaceEditPreviewDecision.openConflict(uri)`。该完成项属于 `PLAN.md` 阶段 4 的 `feat(app): open workspace edit conflict targets` 增量；`SWIFT-GAPS.md` 仅作为任务范围参考，提交归属以 `PLAN.md` 对应章节为准。

AttoEditor App 侧现在会处理 `openConflict` decision：关闭 preview，不推进 core apply transaction，不修改文件，直接打开或选中冲突目标 tab，并给出 `Opened WorkspaceEdit conflict: <file>` 状态反馈。该阶段只提供手动 resolution 入口，不自动保存/丢弃 dirty buffer、不重跑 LSP request、不重新 apply WorkspaceEdit，也不实现跨 transaction conflict resolution 或 project-level conflict owner。

## 阶段 352: WorkspaceEdit save-conflict resolution UI 起点

2026-08-03 阶段 352 已让 WorkspaceEdit preview panel 对 `dirty_document` / `save_or_discard` conflict 提供 `Save Conflict` 操作。`AttoWorkspaceEditPreview` 新增 saveable conflict target helper，panel 新增稳定 accessibility identifier，App 侧新增 `AttoWorkspaceEditPreviewDecision.saveConflict(uri)` handler，复用既有 `saveTabWithSavePanelIfNeeded(_:)` 路径保存冲突目标 tab。该完成项属于 `PLAN.md` 阶段 4 的 `feat(app): save workspace edit conflict targets` 增量；`SWIFT-GAPS.md` 仅作为任务范围参考。

该阶段只解决“保存冲突目标”这个人工动作：保存成功后关闭 preview、返回 false、不推进当前 core transaction event，也不自动重跑 LSP request 或重新 apply。测试覆盖保存目标 tab 后 dirty 状态清除、磁盘内容写入，以及用户随后手动重试同一 WorkspaceEdit transaction 可以成功应用。

## 阶段 353: WorkspaceEdit discard-conflict resolution UI 起点

2026-08-03 阶段 353 已让 WorkspaceEdit preview panel 对同一类 `dirty_document` / `save_or_discard` conflict 提供 `Discard Conflict` 操作。`AttoWorkspaceEditPreview` 新增 discardable conflict target helper，panel 新增稳定 accessibility identifier，App 侧新增 `AttoWorkspaceEditPreviewDecision.discardConflict(uri)` handler，从磁盘重载冲突目标 tab，并通过既有 open-tab text replacement / core dirty sync 路径清除 dirty 状态。该完成项属于 `PLAN.md` 阶段 4 的 `feat(app): discard workspace edit conflict targets` 增量；`SWIFT-GAPS.md` 仅作为任务范围参考。

该阶段同样只解决人工动作：丢弃成功后关闭 preview、返回 false、不推进当前 core transaction event，也不自动重跑 LSP request 或重新 apply。测试覆盖丢弃后 tab 文本恢复为磁盘内容、dirty 状态清除，以及用户随后手动重试同一 WorkspaceEdit transaction 可以成功应用。

## 阶段 354: WorkspaceEdit resolved-conflict retry UI 起点

2026-08-03 阶段 354 已把 save/discard conflict resolution 从“用户手动解决后再自己重试”推进到显式 retry 动作：preview panel 新增 `Save & Retry` / `Discard & Retry` 按钮和稳定 accessibility identifier，分别返回 `AttoWorkspaceEditPreviewDecision.saveAndRetry(uri)` / `.discardAndRetry(uri)`。该完成项属于 `PLAN.md` 阶段 4 的 `feat(app): retry workspace edits after resolving conflicts` 增量；`SWIFT-GAPS.md` 仅作为任务范围参考。

App 层的 WorkspaceEdit preview confirmation 现在内部使用 `apply/stop/retry` 三态结果；retry 成功解决 dirty conflict 后，会重新同步打开 tab 到 core、重新 preview 同一 WorkspaceEdit JSON，并在后续 confirmation 允许时继续走 core transaction apply。该阶段仍不重跑 LSP request、不重新生成 WorkspaceEdit payload，也不实现跨 transaction conflict resolution 或 project-level conflict owner。
...
- rename 主路径已有 App 输入 UI、prepareRename range/placeholder 默认名、prepare/result typed payload wrapper、当前文档 WorkspaceEdit 应用、跨文件 WorkspaceEdit 摘要预览、打开 tab / 本地 `file://` 文档 text edits 应用、打开 tab / 本地未打开文件 resource operations、App 层跨 family event stream 接入和阶段 196-221 core WorkspaceEdit transaction 起点/root-gated 本地文件覆盖/App apply helper 主路径/基础 preview confirmation/专用 diff preview panel 起点/未打开文件 text edit rollback、open-tab rollback、atomic apply mode、atomic runtime failure rollback、resource-order dependency preflight、ordered unsupported dependency preflight、resource operation typed summary、dirty/conflict typed summary、打开 tab undo grouping 起点、最近一次 transaction undo 起点、用户级 command/menu/keymap 起点、专门 history panel 起点和 bounded 多级 transaction-wide undo stack、redo 已产品化；仍缺更深层 conflict 检测/解决 UI 和 request lifecycle。
...
- code action 主路径已有 App quick panel、resolve、typed diagnostics context、result/resolve typed payload wrapper、App 主路径 typed payload 消费、kind/filter、当前文档 edit 应用、跨文件 WorkspaceEdit 摘要预览、打开 tab / 本地 `file://` 文档 text edits 应用、打开 tab / 本地未打开文件 resource operations、command 执行、执行结果/错误 HUD、App 层跨 family event stream 接入和阶段 196-221 core WorkspaceEdit transaction 起点/root-gated 本地文件覆盖/App apply helper 主路径/基础 preview confirmation/专用 diff preview panel 起点/未打开文件 text edit rollback、open-tab rollback、atomic apply mode、atomic runtime failure rollback、resource-order dependency preflight、ordered unsupported dependency preflight、resource operation typed summary、dirty/conflict typed summary、打开 tab undo grouping 起点、最近一次 transaction undo 起点、用户级 command/menu/keymap 起点、专门 history panel 起点和 bounded 多级 transaction-wide undo stack、redo 已完成；仍缺更深层 conflict 检测/解决 UI。
...
- `MultiDocumentEditorUi` 已通过 UI FFI/Swift 暴露基础 tab/preview/pin/close/split/search-all-tabs API；AttoEditor 已开始把 tab/pane lifecycle、tab order、preview replacement、活动文本、保存/dirty 状态同步到 core mirror，Find in Files opened scope 已消费 core open-tab search，session restore 会恢复 split pane 数量和 active pane 到 core snapshot，关闭/覆盖/删除保护条件也已优先消费 core dirty snapshot。仍缺 project/LSP lifecycle 等更高层 workspace 契约，以及完整 AttoEditor 迁移。
...
- 配置 DTO 已有 `AttoConfigurationSnapshot` / `AttoCapabilitySnapshot` 起点，覆盖当前有效 editor/rendering/language/workspace 偏好、UI FFI ABI/features、LSP capability 摘要和 platform/App capability；`AttoConfigurationSettings` 已提供 user/workspace/runtime partial overlay 合并、settings JSON store 起点和 `scoped_settings` typed overlay；AttoEditor App 创建窗口和偏好重应用路径已消费 user/workspace/runtime settings 的 resolved snapshot，并会按打开文档的 URL + language id 对 tab/pane 应用 scoped editor/language/rendering settings，包括 document/tab 级 `rendering.theme_name` 主题切换；AttoEditor 已有打开/创建 user settings 与 workspace `.attoeditor/settings.json` 的命令/菜单入口，settings JSON 编辑复用现有 tab/core mirror 路径；损坏 settings 文件会备份为 `*.invalid` 后被忽略；legacy v0 settings 会备份并写回 current schema；Find bar 默认大小写、整词和 regex 搜索选项、Find in Files 默认 scope、workspace search include/exclude glob、自定义 ASCII word-boundary chars 已纳入配置并应用到 Swift UI / Rust whole-word search/find/replace/search-highlight 路径；全局 Preferences UI 已覆盖默认 Find 选项、Find in Files 默认 scope 和 workspace search include/exclude glob。但完整 Sublime settings selector grammar、schema-aware settings 表单/校验/补全 UI、runtime override UI/持久化、跨 schema 字段语义迁移等仍不完整。
- headless Swift FFI 已有 ABI version、feature flags、`runtimeInfo()` typed facade 和 runtime mismatch / missing feature diagnostic 起点；UI Swift FFI 也已有可复用的 runtime mismatch / missing feature compatibility report；headless/UI C ABI 已有 C/非 Swift host 可用的一次性 runtime info JSON snapshot；阶段 69 已补齐 UI FFI 的 ABI version / feature flags C ABI 和 Swift `runtimeInfo()` typed facade，阶段 80 已新增 multi-document UI feature flag，阶段 81 已把该 feature 纳入 AttoEditor 启动期必需能力；阶段 70 已补 AttoEditor 启动期最低 ABI/必需 feature compatibility gate，阶段 105 已补基础逐命令可选 feature 降级；阶段 10 已为 UI FFI JSON command dispatcher、headless `EditorState` / `Workspace` JSON command bridge、headless `EditorState` full-state / text / line-ending / text-delta query 面、headless `Workspace` open-buffer / create-view lifecycle 面、headless `Workspace` search-all-open-buffers / apply-text-edits result 面、headless `Workspace` info / buffer-text / viewport-state query 面、headless LSP URI/path/formatting/normalization helper 面、UI FFI LSP take-last result slots、per-EditorUi/MultiDocument event streams、per-`EditorUi` derived-state / LSP status / LSP WorkspaceEdit / minimap / view-point payload hit-test result 面、MultiDocument workspace diagnostics / WorkspaceEdit transaction 专用事件流、MultiDocument WorkspaceEdit transaction preview/apply/undo result 面、MultiDocument workspace diagnostics apply/snapshot/markers/previous-result-ids result 面、MultiDocument workspace outline snapshot result 面、MultiDocument tab/project snapshot result 面、MultiDocument all-tabs search result 面、MultiDocument workspace roots change result 面，以及 MultiDocument project LSP server config result 面增加 `{ ok, value, error, version }` envelope API / Swift typed wrapper 起点，且命令、LSP result、event stream、WorkspaceEdit transaction envelope、workspace diagnostics envelope、workspace outline snapshot envelope、multi-document snapshot envelope、multi-document search envelope、workspace roots change envelope、project LSP servers envelope、per-`EditorUi` minimap envelope、view-point payload envelope、headless workspace result/query/lifecycle envelope、headless editor-state query envelope 与 headless LSP helper envelope 的 unknown future fields / unknown status Swift decode 兼容性已有回归覆盖。后续仍缺其他 JSON result 面的统一 envelope 覆盖、更细粒度的逐面板降级策略和面向第三方 host 的更完整 ABI capability negotiation。
...
- FFI 返回统一 `{ ok, value, error, version }` 风格；UI command JSON、headless command JSON、headless workspace/editor-state query/result/lifecycle JSON 以及 headless LSP helper JSON 已有兼容 envelope 起点，后续继续扩到其他 core/helper JSON 结果面。
...
- 已完成：AttoEditor 配置 DTO 和 settings JSON 已覆盖自定义 ASCII word-boundary chars，并应用到 Swift UI / Rust whole-word search、find/replace、search highlight 和 multi-document search 路径。
...
- 已完成：AttoEditor 状态栏已有语言来源 indicator，能区分 LSP semantic tokens、LSP + Tree-sitter、Tree-sitter、Sublime syntax baseline 和 Plain Text；该路径明确把新增语言能力展示导向 Tree-sitter/LSP 主线，Sublime syntax 只作为 fallback baseline 呈现。
- 已完成：Swift/App 配置快照、settings 文件和 preferences 已提供 `semantic_highlighting_enabled` 开关；AttoEditor typed semantic tokens apply 主路径会尊重该开关，关闭后跳过新的 typed semantic tokens 结果并清空已应用 semantic token style layer。该完成项属于 `PLAN.md` 阶段 11 的 `feat(lang): configure semantic highlighting` 增量；`SWIFT-GAPS.md` 仅作为任务范围参考。
- 已完成：Swift/App 配置快照、settings 文件和 preferences 已提供 `format_on_save_enabled` 开关；AttoEditor 保存路径会在写盘前按配置 best-effort 调用 LSP document formatting，成功时保存格式化后的文本，formatter 不可用/失败/无 edits 时不阻断保存。该完成项属于 `PLAN.md` 阶段 11 的 `feat(lang): format on save from settings` 增量；`SWIFT-GAPS.md` 仅作为任务范围参考。
- 已完成：Swift/App 配置快照、settings 文件和 preferences 已提供 `format_on_type_enabled` 开关；AttoEditor 新建 editor chrome 和 preference 重应用路径会把该开关下发到 core UI，关闭后自动 trigger-character on-type formatting 不再发起 `textDocument/onTypeFormatting`，但 LSP didChange 同步和显式 `lsp_format_on_type(...)` API 保持可用。该完成项属于 `PLAN.md` 阶段 11 的 `feat(lang): configure on-type formatting` 增量；`SWIFT-GAPS.md` 仅作为任务范围参考。
- completion commit-time resolve、rich documentation/detail preview、commitCharacters 提交行为、server triggerCharacters 自动触发、本地增量过滤、普通 completion additional edits core transaction/request retry owner、跨文件 WorkspaceEdit 摘要预览、打开 tab / 本地 `file://` 文档 text edits 应用、打开 tab / 本地未打开文件 resource operations、App 层跨 family event stream 接入、Swift UIFFI completion result / resolve item typed payload wrapper、completion App 主路径 typed payload 消费和阶段 196-221 core WorkspaceEdit transaction 起点/root-gated 本地文件覆盖/App apply helper 主路径/基础 preview confirmation/专用 diff preview panel 起点/未打开文件 text edit rollback、open-tab rollback、atomic apply mode、atomic runtime failure rollback、resource-order dependency preflight、ordered unsupported dependency preflight、resource operation typed summary、dirty/conflict typed summary、打开 tab undo grouping 起点、最近一次 transaction undo 起点、用户级 command/menu/keymap 起点、专门 history panel 起点、history panel conflict target owner 起点、history panel save/discard conflict resolution 起点和 bounded 多级 transaction-wide undo stack、redo 已完成；仍缺 snippet placeholder 与 transaction 深度合并语义，以及更深层 conflict 检测/解决 UI。
...
- `MultiDocumentEditorUi` 基础 FFI/Swift 投影已完成，覆盖 open/select/close/pin/preview/split/view move/tab move/search-all-tabs、tab 文本同步、dirty/saved snapshot、tab document URI 与 language id metadata、workspace diagnostics store、workspace diagnostic marker snapshot、workspace diagnostics event stream 起点、MultiDocument/project 级 result event 聚合起点和 MultiDocument/project 级 request event 聚合起点；AttoEditor tab/pane lifecycle、tab order、编辑文本、语言 metadata、workspace Problems snapshot、workspace markers、workspace/active diagnostics 状态栏摘要、active-tab/workspace Problems list、diagnostics lifecycle entry、events-after 查询、stale/refresh lifecycle metadata 和 App 层跨 family event stream 起点已开始同步到 core-backed 投影，Find in Files opened scope 已改为 core open-tab search，split pane session restore、pane move、tab move、tab drag reorder、preview replacement/close callback、active-tab reload/save、save-all dirty tabs、active-tab pin、close-active fallback、active-pane focus/move/close、dirty close confirmation 和 dirty/close/resource-operation 保护条件已同步到 core snapshot；`EditorUi` 已有 core-owned result slot event stream 起点、request lifecycle event stream 起点、显式 cancel/timeout lifecycle、on-type formatting request lifecycle、UI auxiliary derived-state request lifecycle、semantic/folding internal refresh request lifecycle、Swift event metadata typed accessor 和 pull diagnostics typed payload envelope；workspace root 变更和 session restore 后已能基于 core-projected open tabs 自动批量启动可配置但尚未启用的 LSP session，LSP process status 已包含 running/exited/exit code/signal 和 bounded stderr tail，project status events 面板可展示 failed status 的 stderr tail，Swift/App 侧已有 bounded project LSP process health history、轻量 health 面板、dashboard 起点和 summary/recovery policy/inline recovery action/server grouping/trend summary、JSONL 持久化日志、按 workspace retention、大小/时间轮转、filter DSL、显式 log 查询面板、当前 workspace 清空入口/确认、当前 workspace 导出入口和 failed/exited 可配置自动重启/退避/参数调整起点，并可在 dashboard 中按 server 禁用/启用 auto-restart、调整 max attempts/base delay、重置回全局策略；`MultiDocumentEditorUi` 也已有 project-level LSP server launch metadata store / FFI / Swift wrapper 起点，AttoEditor 已开始把打开 tabs 的 `lspServerConfig` 同步到该 core store，launch metadata 的 `workspaceRoots` 已优先跟随 core workspace snapshot，core 现在能基于 open tab document URI、language id 和 project LSP server configs 生成 project LSP auto-start/restart/stop plan，AttoEditor project/root 自动启动路径已消费 start plan，active-tab manual restart、project restart 和 failed/exited auto-restart 路径已消费 restart plan，active-tab manual shutdown 和 project shutdown 路径已消费 stop plan，active-tab manual restart/shutdown plan gate 拒绝已写回 skipped lifecycle outcome，通过 plan gate 后会先写回 requested lifecycle event，auto-start、active-tab、project-level restart/shutdown、auto-restart 与用户级 stop 的 requested 和终态 outcome 也已有 `attempt_id` 关联，并能把自动启动、restart、用户级 stop、active-tab manual shutdown 与 project shutdown 的 lifecycle outcome 写回 core lifecycle event stream；Project Status Events panel / Project Dashboard 也已消费并展示这些 lifecycle outcomes，且可显示 lifecycle attempt id 和 attempts summary。仍缺把实际启动/restart/stop/shutdown 执行和进程 ownership 下沉为 core-owned typed lifecycle、更完整 project/LSP ownership schema、跨独立 project session 策略和完整 dashboard 产品化。
...
- 继续收敛 session restore、dirty state、close semantics 和更高层 workspace 命令；preview replacement、pin tab、dirty/close 保护条件和 tab movement 已经 core-backed，剩余重点是把更高层关闭命令、tab drag/drop 和 project/session 归属完全转为 core workspace command/query。
- 明确 project/workspace 与 LSP server lifecycle 由 core workspace 模型协调，Swift 只负责启动参数、UI 触发和展示；当前 root diff、workspaceFolders 通知、shared root alias、project restart/shutdown/close、root 变更/session restore 后的自动启动、stderr tail 状态、项目级失败事件展示、bounded 进程健康历史、轻量 health 面板、dashboard 起点和 summary/recovery policy/inline recovery action/server grouping/trend summary、JSONL 持久化日志、按 workspace retention、大小/时间轮转、filter DSL、显式 log 查询面板、当前 workspace 清空入口/确认、当前 workspace 导出入口、failed/exited 可配置自动重启/退避/参数调整起点、dashboard 内 server-level auto-restart 禁用/max/base 调整/reset 入口、core-owned project LSP server launch metadata store + App projection 同步、launch metadata root 列表跟随 core workspace snapshot、core tab language metadata、core-owned project LSP start/restart/stop plan 查询、App 自动启动消费 start plan、active-tab manual restart、project restart 和 failed/exited auto-restart 消费 restart plan、active-tab manual shutdown 和 project shutdown 消费 stop plan、auto-start/restart/user stop/active-tab manual shutdown/project shutdown outcome、auto-start requested/attempt outcome、active-tab restart/shutdown plan-gate skipped/requested/attempt outcome、project-level restart/shutdown requested/attempt outcome、auto-restart requested/attempt outcome 与用户级 stop requested/attempt outcome 写回 core lifecycle event stream，以及 status/dashboard 消费 lifecycle outcome、attempt id 展示和 attempts summary 已有起点，剩余重点是把 typed lifecycle 执行、启停 ownership、恢复策略和健康视图继续下沉/产品化。
- 建立迁移期测试：同一 tab/split/workspace 操作同时断言 core workspace snapshot、Swift wrapper query 和 AppKit 投影，避免两边状态漂移。

### P2：深化 Sublime 兼容

- settings scopes：已有 typed resolver 和 App per-tab 应用起点，支持 user/workspace/runtime scoped overlay 按文档 URL、文件名/扩展名、语言 id 和简化 `source.*` / `text.*` selector 匹配，并已覆盖 scoped editor/language/rendering theme 应用；已有打开/创建 settings JSON 的命令/菜单入口，仍缺完整 Sublime settings selector grammar 和 schema-aware 编辑 UI。
...
- 已完成：阶段 13 已扩展 opt-in `XCUIApplication` injected result panel navigation smoke flow，在 injected Locations panel 中通过方向键和 Return 打开第二个 location，在 Document Symbols 与 Workspace Outline panel 中搜索 `smokeChild` 后 Return 打开目标，并用 status bar position AX label 断言跳转行列。该完成项属于 `PLAN.md` 阶段 13 的 `test(app): cover xcui result panel navigation` 增量；`SWIFT-GAPS.md` 仅作为任务范围参考。
- 已完成：阶段 13 已新增 opt-in `XCUIApplication` real stdio LSP result flow，测试侧写入最小 Python fixture LSP server 并通过 `ATTO_EDITOR_LSP_CMD` 注入 App，打开 `.rs` 文件后等待真实 `didOpen`，经 command palette 发送 `definition` 和 `documentSymbol` 请求，断言 Locations、Document Symbols 与 Workspace Outline panel 由真实 server response 填充，并通过 row open/status bar position 验证目标跳转。该完成项属于 `PLAN.md` 阶段 13 的 `test(app): cover xcui real lsp result flow` 增量；`SWIFT-GAPS.md` 仅作为任务范围参考。
...
- 已完成：阶段 13 已扩展持久在线 result panel visual matrix，manifest 可结构化注入 active diagnostics 与 workspace diagnostics，打开并捕获 active Problems、Workspace Problems 和 LSP Workbench panel，并新增三个 dark artifact case。该完成项属于 `PLAN.md` 阶段 13 的 `test(app): cover visual problems workbench panels` 增量；`SWIFT-GAPS.md` 仅作为任务范围参考。
- 已完成：阶段 13 已扩展持久在线 result panel visual matrix，manifest 可结构化生成 incoming/outgoing/type hierarchy result JSON，复用 App hierarchy snapshot 路径打开并捕获 Hierarchy panel，并新增 dark incoming-call Hierarchy panel artifact case。该完成项属于 `PLAN.md` 阶段 13 的 `test(app): cover visual hierarchy panel` 增量；`SWIFT-GAPS.md` 仅作为任务范围参考。
- 已完成：阶段 13 已扩展持久在线 auxiliary LSP panel visual matrix，manifest 可结构化生成 Code Lens、Inlay Hints、Document Links 和 Document Colors 结果，并复用 App derived-state/result apply 路径打开并捕获对应 panels，新增四个 dark artifact case。该完成项属于 `PLAN.md` 阶段 13 的 `test(app): cover visual auxiliary lsp panels` 增量；`SWIFT-GAPS.md` 仅作为任务范围参考。
- 已完成：阶段 13 已扩展 popup visual matrix，manifest 可直接生成并捕获 WorkspaceEdit failure 与 LSP request failure feedback popover content view，并新增两个 dark artifact case。该完成项属于 `PLAN.md` 阶段 13 的 `test(app): cover visual failure popovers` 增量；`SWIFT-GAPS.md` 仅作为任务范围参考。
- 已完成：阶段 13 已扩展 WorkspaceEdit visual matrix，manifest 可结构化生成 WorkspaceEdit transaction preview 和 preview panel sections，并通过 package-internal non-modal testing/visual entry 捕获 modal WorkspaceEdit preview panel content view，新增 dark WorkspaceEdit preview panel artifact case。该完成项属于 `PLAN.md` 阶段 13 的 `test(app): cover visual workspace edit preview` 增量；`SWIFT-GAPS.md` 仅作为任务范围参考。
...
- 已完成：阶段 13 已扩展 WorkspaceEdit version mismatch failure visual matrix，manifest 可生成带 `textDocument.version` 的真实 WorkspaceEdit JSON，断言打开 tab 文本保持原文，并捕获 core document version preflight mismatch 后生成的 summary popover，新增 dark version mismatch failure artifact case。该完成项属于 `PLAN.md` 阶段 13 的 `test(app): cover visual workspace edit version mismatch` 增量；`SWIFT-GAPS.md` 仅作为任务范围参考。
- 已完成：阶段 13 已扩展 WorkspaceEdit resource conflict partial visual matrix，manifest 可真实执行打开 tab text edit 后再执行同 URI `create` resource operation，断言打开 tab 文本保持已应用状态，并捕获 resource operation target conflict 后生成的 partial summary popover，新增 dark resource conflict partial artifact case。该完成项属于 `PLAN.md` 阶段 13 的 `test(app): cover visual workspace edit resource conflict` 增量；`SWIFT-GAPS.md` 仅作为任务范围参考。
- 已完成：阶段 13 已扩展 WorkspaceEdit resource dependency failure visual matrix，manifest 可用 `orderedChanges` 按 LSP `documentChanges` 顺序交错生成 resource operation 和 text edit，真实执行同 URI `create` resource operation conflict 后被 dependency preflight 阻断的 text edit，断言打开 tab 文本保持原文，并捕获 summary popover，新增 dark resource dependency failure artifact case。该完成项属于 `PLAN.md` 阶段 13 的 `test(app): cover visual workspace edit dependency failure` 增量；`SWIFT-GAPS.md` 仅作为任务范围参考。
- 已完成：阶段 13 已新增专门的 WorkspaceEdit/global undo history UI 起点，AttoEditor 通过 `workspace.show_workspace_edit_history` command 打开 WorkspaceEdit History panel，panel 直接消费 core-owned `MultiDocumentEditorUI` WorkspaceEdit transaction event stream，并显示 sequence、operation、Applied/Partial/Rejected 状态、edit/resource operation 数量和 URI 摘要；visual manifest 新增真实 WorkspaceEdit apply 后捕获 dark history panel 的 artifact case。该完成项属于 `PLAN.md` 阶段 13 的 `feat(app): show workspace edit history panel` 增量；`SWIFT-GAPS.md` 仅作为任务范围参考。
- 已完成：阶段 13 已硬化 visual baseline runner preflight：capture/record/strict 路径会在无 Metal 环境下提前 skip 或失败并给出明确诊断，`check-visual-baselines.sh` / `update-visual-baselines.sh` 共享 SwiftPM 参数 helper，可通过 `ATTO_VISUAL_SWIFTPM_*` 把 scratch/cache/config/security/sandbox 配置指向受限 runner 可写路径。该完成项属于 `PLAN.md` 阶段 13 的 `test(app): harden visual baseline runner preflight` 增量；仍不提交 PNG golden baseline，也不启用 strict 默认 PR 门禁。
- 已完成：阶段 13 已预置 strict visual baseline PR 门禁：visual-baselines workflow 会在所有 PR 上检测 checked-in `VisualBaselines/*.png`，无 PNG 时继续 smoke artifact capture，有首批 PNG 后自动运行 strict PNG comparison，使同一个 macOS job 成为合入后的默认视觉门禁。该完成项属于 `PLAN.md` 阶段 13 的 `ci(app): arm strict visual baseline gate` 增量；仍不提交 PNG golden baseline。
- 已完成：阶段 13 已扩展 WorkspaceEdit runtime rollback failure 视觉覆盖：core transaction apply 抛出的资源操作 runtime failure 现在会显示稳定 WorkspaceEdit failure popover，visual manifest 新增 create + overwrite rename + failing create 的 resource rollback case，并断言 rollback 后已创建文件/目录被移除、原文件内容恢复。该完成项属于 `PLAN.md` 阶段 13 的 `feat(app): surface workspace edit runtime rollback failures` 增量；仍不提交 PNG golden baseline，也不改变 core transaction schema/apply 语义。
- 已完成：阶段 13 已扩展 WorkspaceEdit unsupported operation partial summary 覆盖：App apply summary 现在消费 core transaction result 的 `unsupportedOperationURIs`，当 workspace 内 text edit 成功但 workspace 外 resource operation 被拒绝时会展示部分应用摘要；visual manifest 新增 dark unsupported resource operation partial summary artifact case。该完成项属于 `PLAN.md` 阶段 13 的 `feat(app): summarize unsupported workspace edit operations` 增量；仍不提交 PNG golden baseline，也不改变 core transaction schema/apply 语义。
- 已完成：阶段 13 已扩展 WorkspaceEdit skipped detail failure summary 覆盖：App apply summary 现在消费 core transaction result 的 `skippedDetails`，version mismatch 等跳过原因会出现在失败 popover 的文档行中；既有 dark version mismatch failure artifact case 会经真实 App apply 路径覆盖该展示。该完成项属于 `PLAN.md` 阶段 13 的 `feat(app): show workspace edit skipped reasons` 增量；仍不提交 PNG golden baseline，也不改变 core transaction schema/apply 语义。
- 仍缺提交首批经批准机器生成的 PNG golden baselines。
- 继续扩展 fixtures，覆盖 WorkspaceEdit rollback secondary failure 本身再次失败的极端路径、更多 conflict/failure 边界变体，以及经审核的跨 theme/window-size 真实 baseline 场景。
- 继续扩展 `XCUIApplication` smoke tests，覆盖外部真实语言服务器（如 rust-analyzer/sourcekit-lsp）矩阵、多文件 workspace、多 root/project session，以及真实 server 错误/延迟/重启后的 Locations/Symbols/Workspace Outline panel 行为。
...
| LSP formatting | yes | partial helper | yes, document/range/on-type blocking apply + trigger-character auto path + document/range raw request/take | yes, document/range/on-type blocking apply + document/range raw request/take | yes, typed document/range/on-type helpers + formatting outcome + document/range raw result wrappers | document + selection commands with no-edits/error HUD and WorkspaceEdit transaction apply/request retry owner; on-type trigger-character auto path | partial |
...
| workspace tabs/splits | yes, headless `Workspace` | partial `Workspace` wrapper with view-local command conveniences | yes, `MultiDocumentEditorUi` | yes, `MultiDocumentEditorUi` ABI + close-view + move-view + move-tab + text/dirty sync | partial `MultiDocumentEditorUI` wrapper; Atto tab/pane lifecycle, tab move/drag reorder, pane move, preview replacement, preview replacement close callback, active-tab reload/save, save-all dirty tabs, active-tab pin, close-active fallback, active-pane focus/move/close, dirty close prompt and edited text mirror core snapshot/search, but session/project/LSP ownership is still transitional | partial, transitional AppKit projection; new tab/workspace semantics must move to core-owned workspace first | partial |

阶段 221 后，矩阵中 `WorkspaceEdit` 的 core transaction 起点已从 open-tab 覆盖扩展到 root-gated 未打开本地文件 text edits/resource operations，并按 `documentChanges` 顺序交错应用 text edits 与 resource operations；未打开本地 resource operation、未打开文件 text edit 和打开 tab 状态变更的 fatal failure 已有补偿回滚起点，打开 tab 的本地 resource operation 也已开始由 core transaction 执行 root-gated 文件系统副作用；AttoEditor 的 WorkspaceEdit apply helper 主路径也已切到 `MultiDocumentEditorUI.applyWorkspaceEditTransaction(...)`，并有基础 preview confirmation、专用 diff preview panel 起点、atomic runtime failure rollback、resource-order dependency preflight、ordered unsupported dependency preflight、resource operation typed summary、dirty/conflict typed summary、打开 tab App 投影 undo grouping 起点、用户级 command/menu/keymap、专门 history panel、history panel conflict target owner 起点、history panel save/discard conflict resolution 起点、history transaction reapply/save-and-reapply 起点、history request rerun/save-and-rerun 起点、bounded 多级 transaction-wide undo stack、redo、conflict preview 分组/partial apply 按钮提示、core conflict impact/resolution schema、transaction envelope parser compatibility、blocking conflict App apply gate、open-conflict resolution UI 起点、save/discard-conflict 手动解决动作、resolved-conflict 显式 retry 动作以及 rename/code action/completion/formatting/execute-command request retry owner 起点；普通 completion additional edits、snippet completion additionalTextEdits transaction + placeholder session 保持起点、显式 document/range formatting 和 executeCommand result WorkspaceEdit 已开始走 WorkspaceEdit transaction。仍未完成的是真正跨 session/project-level request/conflict owner 持久化或共享、snippet 主体与 transaction 的统一 undo/跨文件合并语义和更深层 conflict 检测/解决 UI。
