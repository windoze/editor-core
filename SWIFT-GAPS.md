...
本阶段继续保留本地 fallback diagnostics 到 marker projection 的路径，避免旧 runtime 或 core marker snapshot 失败时丢失基础 marker 展示。测试覆盖初次 marker 读取、无新事件重复读取不刷新、直接 core diagnostics 更新后 marker projection 通过 event cursor 刷新到新位置/严重级别，以及重复读取继续命中缓存。阶段 188 已把 Locations/Symbols 持久在线 panel 接到 lifecycle entry；剩余状态订阅缺口不再包括 workspace Problems/marker 的基础 cursor 消费，也不再包括 Locations/Symbols panel 的 active-edit stale 展示，而是集中在 workspace/project 级 outline store 和更完整 project-level result lifecycle/error 聚合。
...
本阶段不改变 panel 列表、过滤、导航或 history 行为，只为后续 stale/error 状态、project-level refresh metadata 和 panel event cursor 展示提供 UI 锚点。测试覆盖 controller 级稳定 identifier 与 snapshot metadata 文本，以及真实 Locations/Symbols App 路径在新结果自动刷新时 metadata label 随 lifecycle entry sequence/title 同步更新。阶段 190 已继续接入 active-edit stale 状态；阶段 191 已接入 active App request feedback error 状态；剩余工作是把 project/workspace 级 outline store 和 project-level error 聚合接入这条 metadata UI。
...
本阶段覆盖的是 stale 状态的产品化和 Error 状态的模型/UI 承载位。测试覆盖 lifecycle store 状态更新与 history 同步、snapshot 兼容 metadata 文本、Locations panel 与 Symbols panel 在文档编辑后从 Fresh 变为 Stale、以及新结果刷新后恢复 Fresh。阶段 191 已继续把 Locations/Symbols request feedback error 接到 panel current entry；剩余工作是继续推进 project/workspace 级 outline store 与 project-level result lifecycle/panel 整合。
...
本阶段覆盖的是 active App 路径中的 current Locations/Symbols panel error 状态；不改变 quick panel、history、导航或 result event stream ABI。测试覆盖已有 Locations panel / Symbols panel 在 fresh result 后触发 LSP unavailable request，current entry 保持原 sequence/snapshot 并转为 Error metadata。剩余工作是把 project-level request/result event cursor 的错误聚合、workspace/project 级 outline store 和 core-owned WorkspaceEdit transaction 继续收敛。
...
- definition/declaration/type definition/implementation/references 多结果已有统一 quick panel，展示 item model、稳定排序、最近结果 snapshot、reopen command、bounded in-memory history command 和基础持久在线 Locations/References panel；locations/references 的 current/history 已迁入通用 lifecycle store，已接入 App 层跨 family event stream，持久在线 panel 也已开始消费 lifecycle entry。仍缺项目级归属、project-level error 聚合和更完整刷新/过期策略。
...
- outline / document symbols 已有 quick panel 主路径，document/workspace symbols result typed payload wrapper 和 App 主路径消费已完成，document symbols 展示可消费 typed derived-state snapshot，基础错误/超时/空结果反馈、最近结果 snapshot、reopen command、bounded in-memory history command 和基础持久 Outline/Symbols panel 已补齐；document/workspace symbols 的 current/history 已迁入通用 lifecycle store，已接入 App 层跨 family event stream，持久在线 panel 也已开始消费 lifecycle entry。仍缺 project/workspace 级 outline store、project-level error 聚合和更完整刷新/过期策略。
