...
本阶段仍不是完整 core-owned 跨文件 WorkspaceEdit transaction：未打开磁盘文件的 resource operations、非 open-tab 目标 overwrite/replace、版本检查、conflict detail、atomic rollback、undo grouping、filesystem side effect、以及 AttoEditor rename/code action/completion 主路径切换仍按 `PLAN.md` 后续阶段推进。
