...
`PLAN.md` 是后续工程实施、阶段排序、提交边界、验证命令和最终完成标准的唯一执行计划来源。本文继续作为缺口审计与状态记录，不再承担独立排期职责；随着各阶段完成，需要同步更新对应 gap 的完成证据、剩余风险或 out-of-scope 说明。后续新增、调整或关闭目标时，也应先映射到 `PLAN.md` 的阶段计划，再回写到本文的 gap 状态。
...
`editor-core-ui-ffi` 新增 multi-document WorkspaceEdit transaction C ABI、public header 声明和 `multiDocumentWorkspaceEditTransaction` feature bit；Swift `EditorCoreUIFFI.MultiDocumentEditorUI` 新增 `EcuWorkspaceEditTransactionResult` / `EcuWorkspaceEditTransactionDocument` typed wrapper 和 preview/apply API。返回 JSON 继续兼容 `AttoWorkspaceEditApplyResult` 的核心字段（`applied`、`applied_uri`、`applied_edit_count`、`skipped_uris`、`documents`），并额外提供 `mode`、`applied_uris`、`unsupported_operation_uris`、`is_open`、`tab_id`，供后续 App 主路径切换和 preview panel 使用。

本阶段明确仍不是完整 `PLAN.md` 阶段 4 终态：未打开本地文件、`create` / `rename` / `delete` resource operations、非 `file://` URI、version mismatch、atomic rollback、undo grouping、conflict detail、state-event transaction entry 和 rename/code action/completion/formatting App 主路径切换仍需后续阶段完成。阶段 198 已继续补齐已打开 core tab 的 resource operations 起点。当前 App 层 WorkspaceEdit 应用路径仍保留为迁移期 fallback。测试覆盖 Rust MultiDocument preview/apply、C ABI smoke、Swift typed wrapper/runtime feature 与 skipped/unsupported summary。
...
本阶段仍是专用 transaction event cursor，不代表完整统一 `MultiDocumentStateEvent` transaction envelope 已完成；阶段 198 已先把已打开 core tab 的 resource operations 纳入该 cursor 记录的 transaction result。后续还需要把未打开文件 resource operations、atomic/undo/conflict detail 和 App 主路径切换产生的事务结果统一纳入同一个 project/workspace lifecycle。
...
本阶段仍不是完整 core-owned 跨文件 WorkspaceEdit transaction：未打开磁盘文件的 resource operations、非 open-tab 目标 overwrite/replace、版本检查、更深层 conflict/atomic 诊断、atomic rollback、undo grouping、filesystem side effect、以及 AttoEditor rename/code action/completion 主路径切换仍按 `PLAN.md` 后续阶段推进。阶段 199 已继续补齐 skipped/conflict detail 起点。

## 阶段 199: WorkspaceEdit transaction skipped detail

2026-08-02 阶段 199 已给 `MultiDocumentEditorUi` WorkspaceEdit transaction result 增加机器可读 `skipped_details`：每条 detail 包含 `uri`、`reason`、`operation` 和 `message`。该字段保留现有 `skipped_uris` / `unsupported_operation_uris` 兼容字段，同时为 preview panel、App 主路径切换、transaction event 消费和后续 atomic/conflict UI 提供稳定原因模型。
...
本阶段仍不实现真实 filesystem side effect、version check、atomic rollback 或 undo grouping，也不把 App rename/code action/completion 主路径切到 core transaction；它只补齐“为什么这次 transaction 不能完整应用”的结构化反馈。测试覆盖 Rust MultiDocument detail reason、C ABI JSON 字段和 Swift typed detail decode。
...
- rename prepareRename range/placeholder、prepare/result typed payload wrapper、跨文件 WorkspaceEdit 摘要预览、打开 tab / 本地 `file://` 文档 text edits 应用、打开 tab / 本地未打开文件 resource operations、App 层跨 family event stream 接入和阶段 196 core open-tab WorkspaceEdit transaction 起点已产品化；仍缺完整 core workspace-owned 跨文件事务和 request lifecycle。
- code action typed diagnostics context、result/resolve typed payload wrapper、App 主路径 typed payload 消费、kind/filter、跨文件 WorkspaceEdit 摘要预览、打开 tab / 本地 `file://` 文档 text edits 应用、打开 tab / 本地未打开文件 resource operations、command payload 执行结果/错误展示、App 层跨 family event stream 接入和阶段 196 core open-tab WorkspaceEdit transaction 起点已完成；仍缺完整 core workspace-owned 跨文件事务。
