...
- 明确 project/workspace 与 LSP server lifecycle 由 core workspace 模型协调，Swift 只负责启动参数、UI 触发和展示；当前 root diff、workspaceFolders 通知、shared root alias、project restart/shutdown/close、root 变更/session restore 后的自动启动、stderr tail 状态和项目级失败事件展示已有起点，剩余重点是把更深层 ownership、恢复策略、进程历史和健康视图继续下沉/产品化。
