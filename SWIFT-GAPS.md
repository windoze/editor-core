...
- 阶段 312 继续补齐阶段 7 的统一 LSP Workbench error metadata：App 层已有通用 `markCurrentLspEventResultError(...)`，Workbench 的 event-backed 行可显示 Error；Code Lens、Inlay Hints 和 Document Links 的 refresh unavailable/request failed/timeout/failed/result-error 路径会把最近 result event 标为 Error，Document Colors 与 Hierarchy 已具备同一通用 error state 展示能力。仍缺完整 document color/hierarchy 请求流自动 error propagation、统一 pin/history 数据模型、真正内嵌 dock/workbench 容器和跨 tab/project result history。
