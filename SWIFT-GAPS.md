...
- headless Swift FFI 已有 ABI version、feature flags、`runtimeInfo()` typed facade 和 runtime mismatch / missing feature diagnostic 起点；UI Swift FFI 也已有可复用的 runtime mismatch / missing feature compatibility report；headless/UI C ABI 已有 C/非 Swift host 可用的一次性 runtime info JSON snapshot；阶段 69 已补齐 UI FFI 的 ABI version / feature flags C ABI 和 Swift `runtimeInfo()` typed facade，阶段 80 已新增 multi-document UI feature flag，阶段 81 已把该 feature 纳入 AttoEditor 启动期必需能力；阶段 70 已补 AttoEditor 启动期最低 ABI/必需 feature compatibility gate，阶段 105 已补基础逐命令可选 feature 降级；阶段 10 已为 UI FFI JSON command dispatcher、headless `EditorState` / `Workspace` JSON command bridge、UI FFI LSP take-last result slots、per-EditorUi/MultiDocument event streams、MultiDocument workspace diagnostics / WorkspaceEdit transaction 专用事件流、MultiDocument WorkspaceEdit transaction preview/apply/undo result 面、MultiDocument workspace diagnostics apply/snapshot/markers/previous-result-ids result 面、MultiDocument workspace outline snapshot result 面、MultiDocument tab/project snapshot result 面、MultiDocument all-tabs search result 面、MultiDocument workspace roots change result 面，以及 MultiDocument project LSP server config result 面增加 `{ ok, value, error, version }` envelope API / Swift typed wrapper 起点，且命令、LSP result、event stream、WorkspaceEdit transaction envelope、workspace diagnostics envelope、workspace outline snapshot envelope、multi-document snapshot envelope、multi-document search envelope、workspace roots change envelope 与 project LSP servers envelope 的 unknown future fields / unknown status Swift decode 兼容性已有回归覆盖。后续仍缺其他 JSON result 面的统一 envelope 覆盖、更细粒度的逐面板降级策略和面向第三方 host 的更完整 ABI capability negotiation。
