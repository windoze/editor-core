...
- 阶段 7 后续缺口中，`MultiDocumentEditorUi` 基础 Swift FFI 投影已在阶段 80 补齐，AttoEditor tab/pane lifecycle 到 core multi-document mirror 的迁移起点已在阶段 81 补齐，编辑文本/dirty/search-all-tabs 基础同步已在阶段 83 补齐，Find in Files 的 opened scope 已在阶段 84 开始消费 core open-tab search，split pane 数量/active pane session restore 已在阶段 85 补齐，pane move 已在阶段 86 补齐；仍缺完整 project/LSP lifecycle 迁移、拖拽 tab 到 split、dirty/close semantics 最终统一。
...
- `MultiDocumentEditorUi` 基础 FFI/Swift 投影已完成，覆盖 open/select/close/pin/preview/split/view move/search-all-tabs、tab 文本同步和 dirty/saved snapshot；AttoEditor tab/pane lifecycle 与编辑文本已开始同步到 core mirror，Find in Files opened scope 已改为 core open-tab search，split pane session restore 和 pane move 已同步到 core snapshot；仍缺 project/LSP lifecycle 等更高层 workspace API。
