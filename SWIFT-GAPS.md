...
- `EditorCoreUIFFI.MultiDocumentEditorUI` 已能访问 Rust `MultiDocumentEditorUi` 的基础 tab/preview/pin/close/split/search-all-tabs 模型，并能同步 tab 文本、保存/dirty 状态；AttoEditor session snapshot 保存路径、opened-files/sidebar/tab-bar 投影、active-tab 读取、AppKit content host 投影、tab group-close/close-all 命令以及 opened-files selection/open-existing 查找已开始优先消费 core tab snapshot 中的顺序、active tab、document URI、preview、dirty、view count 和 active view index。
