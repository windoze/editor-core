...
- LSP interactive request 已覆盖一批 raw JSON result API；LSP status/capabilities 已有 typed snapshot，`EditorUi` request lifecycle event stream 起点和 Swift event metadata typed accessor 已补齐，但 request 返回值 payload 仍缺覆盖所有 request family 的统一 typed envelope。
