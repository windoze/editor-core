...
- 阶段 278 继续补齐阶段 6 的 process health log 导出起点：`AttoProjectLspProcessHealthLogStore` 现在可以按当前 workspace root URI 导出 JSONL 记录，AttoEditor 新增 `lsp.export_project_lsp_health_log` 命令、Go 菜单项和 Save Panel 入口；仍缺确认弹窗、复杂查询/filter DSL、按 workspace 独立配额、大小/时间轮转、自动崩溃恢复、更深层 core-owned LSP ownership schema 和完整 dashboard 级健康视图。
- 阶段 279 继续补齐阶段 6 的 process health log 清空确认：`lsp.clear_project_lsp_health_log` 默认命令路径现在会在当前 workspace 有可清空记录时弹出确认 alert，取消不会删除日志；仍缺复杂查询/filter DSL、按 workspace 独立配额、大小/时间轮转、自动崩溃恢复、更深层 core-owned LSP ownership schema 和完整 dashboard 级健康视图。
...
- 明确 project/workspace 与 LSP server lifecycle 由 core workspace 模型协调，Swift 只负责启动参数、UI 触发和展示；当前 root diff、workspaceFolders 通知、shared root alias、project restart/shutdown/close、root 变更/session restore 后的自动启动、stderr tail 状态、项目级失败事件展示、bounded 进程健康历史、轻量 health 面板、JSONL 持久化日志、默认 retention、显式 log 查询面板、当前 workspace 清空入口/确认和当前 workspace 导出入口已有起点，剩余重点是把更深层 ownership、恢复策略、大小/时间轮转和健康视图继续下沉/产品化。
