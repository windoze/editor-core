...
本阶段保留无 workspace root 或非本地 URI 的既有 in-memory open-tab resource operation 行为，避免破坏已有无 root 测试和非文件 URI 场景。它为 AttoEditor App 后续把 WorkspaceEdit apply 主路径切到 core transaction 做准备：core transaction 现在同时覆盖未打开本地文件和打开本地 tab 的 resource operation 磁盘副作用。阶段 207 仍不是完整 `PLAN.md` 阶段 4 终态：AttoEditor App 主路径尚未切换，打开 tab 文本/关闭/URI 状态仍没有 undo grouping，完整 batch atomic rollback、preview panel 产品化和更深层 conflict 检测仍需后续阶段完成。
