...
本阶段仍不是完整状态订阅模型：尚未覆盖所有低层 `lsp_fail(...)` call site、server progress/activity 变化去重、server process exit health event、project-level 多 tab 聚合消费或 UI panel 自动刷新策略。它的边界是建立统一 state event 中的 typed LSP status payload 和首批生命周期/失败触发点。
