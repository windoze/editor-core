...
- 阶段 6 后续缺口中，基础持久在线 references/locations panel 已在阶段 114 补齐，locations/symbols result lifecycle store 起点已在阶段 121 补齐，locations/symbols history entry/envelope 元数据起点已在阶段 122 补齐；多数 result family typed payload 已在阶段 148-161 和阶段 173-174 补齐；阶段 247 已补单 root LSP `workspaceFolders` initialize / client response 起点，阶段 248 已补手动 `workspace/didChangeWorkspaceFolders` 通知与 response 列表更新链路，阶段 249 已补 core-owned root diff 到 AttoEditor active LSP didChange 的接线，阶段 250 已把 root change fan-out 扩展到所有 open-tab LSP sessions，阶段 251 已补保存/关闭路径的 `textDocument/didSave` / `textDocument/didClose` 通知；仍缺 `textDocument/didOpen` 多文档打开生命周期、project open/close 批量 LSP session 启停管理、server restart/shared-session root set 策略、少数 raw-only family typed envelope、统一 feedback/状态订阅和项目级命令模型。
