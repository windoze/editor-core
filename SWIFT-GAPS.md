...
- 阶段 6 后续缺口中，基础持久在线 references/locations panel 已在阶段 114 补齐，locations/symbols result lifecycle store 起点已在阶段 121 补齐，locations/symbols history entry/envelope 元数据起点已在阶段 122 补齐；多数 result family typed payload 已在阶段 148-161 和阶段 173 补齐；仍缺少数 resolve/raw-only family typed envelope、统一 feedback/状态订阅和项目级命令模型。
...
| inlay hints | 阶段 173 已补 manual request/take；自动 auxiliary refresh 与 apply JSON / decorations snapshot 已有 | 阶段 173 已补 public `EcuLspInlayHintResult` wrapper，覆盖 string/label-part labels、kind、tooltip、textEdits、data、error envelope 和 raw payload | renderer inlay、hit-test/caret point 和 status 派生消费已有 | auxiliary refresh lifecycle 已覆盖，manual request 进入 result/request events | 仍缺 `inlayHint/resolve` slot/ABI、统一 feedback 与状态订阅驱动 stale/refresh |
| document links | 阶段 173 已补 manual request/take；自动 auxiliary refresh、apply JSON 和 hit-test 已有 | 阶段 173 已补 public `EcuLspDocumentLinkResult` wrapper，覆盖 target、tooltip、unresolved data、error envelope 和 raw payload | underline/rendering、hit-test/open URL 主路径已有 | auxiliary refresh lifecycle 已覆盖，manual request 进入 result/request events | 仍缺 `documentLink/resolve` slot/ABI、统一 feedback 与状态订阅驱动 stale/refresh |

阶段 173 之后，LSP typed payload 的剩余重点不再是“能否从 Swift 拿到大多数 result family”，而是三件事：少数 resolve/raw-only family 的 typed envelope 上提、所有 family 共享统一 feedback/lifecycle UI、以及把 WorkspaceEdit/状态变更订阅这类跨 family 事实源继续迁到 core/workspace 侧。
...
阶段 163 仍只是统一 feedback 的第一步：hover、signature help、completion、location、symbols、rename/code action、document color、hierarchy 等路径当时仍有各自的 popup/panel/formatter 逻辑；阶段 164-173 已继续迁移 hover typed payload、signature help、location、symbols、completion、document color / color presentation、rename/code action、call/type hierarchy、显式 formatting 命令反馈，以及 inlay hints / document links 的 manual typed request/take wrapper。下一步应继续把 inlay hints、document links 的显式刷新/resolve 行为接入同一个 feedback sink，并让 core-owned request/result/state event stream 驱动 feedback 的生命周期、stale/refresh 展示和 result panel metadata。
...
- App 层已有 request lifecycle/event stream 起点，diagnostics notification/pull refresh lifecycle 已补齐，Swift 侧可 typed 读取 family/slot/phase/status/operation/severity 等事件元数据；派生状态过期响应、增量更新通知、错误展示和状态订阅仍未完全统一。
