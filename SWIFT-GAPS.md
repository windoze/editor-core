...
- 阶段 10 后续缺口中，rich documentation/detail preview 已在阶段 21 补齐，commitCharacters 提交行为已在阶段 22 补齐，server triggerCharacters 自动触发已在阶段 23 补齐；仍缺增量过滤、跨文件 workspace edit 和更完整的 typed result model。
...
- 阶段 12 后续缺口中，rich documentation/detail preview 已在阶段 21 补齐，commitCharacters 提交行为已在阶段 22 补齐，server triggerCharacters 自动触发已在阶段 23 补齐；仍缺增量过滤、跨文件 workspace edit 和更完整的 typed result model。
...
- completion popup 主路径、commit-time completion resolve、rich documentation/detail preview、commitCharacters 提交行为和 server triggerCharacters 自动触发已有；仍缺增量过滤和 completion/code action 中的完整 workspace edit 产品化。
...
- completion commit-time resolve、rich documentation/detail preview、commitCharacters 提交行为和 server triggerCharacters 自动触发已完成；仍缺增量过滤。
...
| LSP completion | yes | partial helper | yes | yes, raw completion + resolve result | yes, raw completion + resolve result | yes, popup + auto trigger + commit-time resolve/current-doc apply | partial |
