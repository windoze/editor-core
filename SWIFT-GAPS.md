...
- 明确 project/workspace 与 LSP server lifecycle 由 core workspace 模型协调，Swift 只负责启动参数、UI 触发和展示；当前 root diff、workspaceFolders 通知、shared root alias、project restart/shutdown/close、root 变更/session restore 后的自动启动、stderr tail 状态、项目级失败事件展示、bounded 进程健康历史、轻量 health 面板和 JSONL 持久化日志已有起点，剩余重点是把更深层 ownership、恢复策略、日志轮转/查询和健康视图继续下沉/产品化。
