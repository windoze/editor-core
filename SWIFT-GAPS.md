...
本阶段保留旧 runtime / 失败路径的 fallback snapshot 行为，并继续让 `apply(resultJSON:)` / typed `apply(result:)` 通过 core-owned workspace diagnostics store 更新 previous result ids、diagnostics 和 Problems panel 输入。测试覆盖直接绕过 store 更新 core diagnostics 后，store 能通过 event cursor 刷新到最新 diagnostics；无新 event 的重复 snapshot 读取不会增加 core snapshot refresh 计数。阶段 187 已继续补齐 marker projection 的同源 cursor 缓存刷新；剩余缺口进一步收窄为 Outline/Symbols/Locations 等持久在线 panel 与 project-level result lifecycle 的统一 cursor 消费，以及 stale/error 状态产品化展示。
...
本阶段继续保留本地 fallback diagnostics 到 marker projection 的路径，避免旧 runtime 或 core marker snapshot 失败时丢失基础 marker 展示。测试覆盖初次 marker 读取、无新事件重复读取不刷新、直接 core diagnostics 更新后 marker projection 通过 event cursor 刷新到新位置/严重级别，以及重复读取继续命中缓存。剩余状态订阅缺口不再包括 workspace Problems/marker 的基础 cursor 消费，而是集中在 Outline/Symbols/Locations 等持久在线 panels、project-level result lifecycle 和 stale/error UI。
