...
- `EditorCoreUIFFI.MultiDocumentEditorUI` 已能访问 Rust `MultiDocumentEditorUi` 的基础 tab/preview/pin/close/split/search-all-tabs 模型，并能同步 tab 文本、保存/dirty 状态；AttoEditor session snapshot 保存路径和 opened-files/sidebar/tab-bar 投影已开始优先消费 core tab snapshot 中的顺序、active tab、preview、dirty、view count 和 active view index。
