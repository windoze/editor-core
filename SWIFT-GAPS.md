...
本阶段不改变 panel 列表、过滤、导航或 history 行为，只为后续 stale/error 状态、project-level refresh metadata 和 panel event cursor 展示提供 UI 锚点。测试覆盖 controller 级稳定 identifier 与 snapshot metadata 文本，以及真实 Locations/Symbols App 路径在新结果自动刷新时 metadata label 随 lifecycle entry sequence/title 同步更新。阶段 190 已继续接入 active-edit stale 状态；剩余工作是把 request error 状态和 project/workspace 级 outline store 接入这条 metadata UI。
...
本阶段覆盖的是 stale 状态的产品化和 Error 状态的模型/UI 承载位，不直接接线具体 LSP request error 到 panel current entry。测试覆盖 lifecycle store 状态更新与 history 同步、snapshot 兼容 metadata 文本、Locations panel 与 Symbols panel 在文档编辑后从 Fresh 变为 Stale、以及新结果刷新后恢复 Fresh。剩余工作是把 request/result error events 映射到 panel entry error 状态，并继续推进 project/workspace 级 outline store 与 project-level result lifecycle/panel 整合。
...
- definition/declaration/type definition/implementation/references 多结果已有统一 quick panel，展示 item model、稳定排序、最近结果 snapshot、reopen command、bounded in-memory history command 和基础持久在线 Locations/References panel；locations/references 的 current/history 已迁入通用 lifecycle store，已接入 App 层跨 family event stream，持久在线 panel 也已开始消费 lifecycle entry。仍缺项目级归属、error 接线和更完整刷新/过期策略。
