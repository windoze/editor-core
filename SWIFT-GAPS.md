...
- 为 request retry owner 增加可持久化、可共享、可审计的 typed descriptor，而不是只依赖当前 session 内的闭包和 label。
- 将 request owner 与 transaction sequence、workspace root、document URI、request kind、原始请求参数、失效原因和可重跑能力关联起来。
