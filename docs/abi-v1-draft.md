...
`server_capabilities`, optional `workspace_roots`, optional `workspace_folders`, and optional
`auto_start`. `language_name` is normalized as display metadata and falls back to the normalized
`language_id` when omitted. `server_capabilities` is a JSON object for host-provided server
capability metadata; it defaults to `{}` and non-object values are rejected. `workspace_folders` is
a typed descriptor list with `uri`, `name`, and optional `root_alias`; when omitted, core derives
...
the tab id, active view index, document URI, language id/name, server capabilities, server key,
command, args, and workspace roots/folders so hosts can execute and record lifecycle outcomes from a
single typed action descriptor instead of inferring ownership from the endpoint used to fetch the
plan.
